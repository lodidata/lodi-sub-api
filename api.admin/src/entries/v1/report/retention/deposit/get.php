...
return new  class() extends BaseController {
    ...
    public function run() {
        ...
        $user_type = $this->request->getParam('user_type', '0');    //用户类型：0-全部，1-新用户，2-老用户
...
//                $user_id_list = \DB::connection('slave')->select($user_id_sql);
    ...
    public function getData($tomorrow_date, $user_id_list, $redis_key){
        ...
        $sql  = "select  '{$tomorrow_date}' date,count(distinct user_id) first_deposit_num, ifnull(sum(money),0) first_deposit_amount from funds_deposit where status='paid' and created >= '{$tomorrow_date} 00:00:00' and created <= '{$tomorrow_date} 23:59:59' and FIND_IN_SET('new',state) and user_id in($user_id_list)";
