...
return new class() extends BaseController {
    ...
    public function run()
    {
        ...
        foreach ($opt as &$v) {
            ...
            $v['ratio'] = empty($v['recharge_cnt']) ? "0%" : (bcdiv(($v['recharge_cnt'] - $v['withdraw_cnt']), $v['recharge_cnt'], 4)) * 100 . "%";   //杀率
        ...
        $f_range            = $this->lang->text('Deposit range');
        $f_user_cnt         = $this->lang->text('No. of people');
        $f_recharge_cnt     = $this->lang->text('Total deposit');
        $f_withdraw_cnt     = $this->lang->text('Total withdrawal');
        $f_withdraw_num     = $this->lang->text('No. of withdrawals');
        $f_bet_user_amount  = $this->lang->text('Total bet');
        $f_profit_diff      = $this->lang->text('Deposit and withdrawal difference');
        $f_ratio            = $this->lang->text('Kill rate');
        $f_recharge_ratio   = $this->lang->text('Ratio of people');
        $f_coin_ratio       = $this->lang->text('Ratio of amount');
        $title = ['range'=>$f_range,'user_cnt'=>$f_user_cnt,'recharge_cnt'=>$f_recharge_cnt,'withdraw_cnt'=>$f_withdraw_cnt,'withdraw_num'=>$f_withdraw_num,'bet_user_amount'=>$f_bet_user_amount,
            'profit_diff'=>$f_profit_diff,'ratio'=>$f_ratio,'recharge_ratio'=>$f_recharge_ratio,'coin_ratio'=>$f_coin_ratio];
        $exp = [];
        foreach ($opt as $k=>$item) {
            $exp[] = [
                'range' => $k,
                'user_cnt' => $item['user_cnt'],
                'recharge_cnt' => $item['recharge_cnt'],
                'withdraw_cnt' => $item['withdraw_cnt'],
                'withdraw_num' => $item['withdraw_num'],
                'bet_user_amount' => $item['bet_user_amount'],
                'profit_diff' => $item['profit_diff'],
                'ratio' => $item['ratio'],
                'recharge_ratio' => $item['recharge_ratio'],
                'coin_ratio' => $item['coin_ratio'],
            ];
        }

        $this->exportExcel("充值用户流水占比",$title, $exp);
        exit();
