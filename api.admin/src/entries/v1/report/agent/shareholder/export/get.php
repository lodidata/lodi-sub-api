...
return new class() extends BaseController
{
    ...
    public function run() {
        ...
        foreach ($game_map as $k=>$v) {
            ...
            $zc_sql .= "TRUNCATE(IFNULL(cast(proportion_list->'$.{$k}' as decimal(15, 2)), 2),0) as zc_{$k} ,";
        }
        ...
        if (!empty($agent_name) && !empty($user_name)) {
            if ($export == 1) {
                $agentInfo = DB::connection('slave')->table('unlimited_agent_bkge')->orderBy('date','desc')->whereRaw('user_name = ? and agent_name = ? and date >= ? and date < ?', [$user_name, $agent_name, $date_start, $date_end])
                    ->get([DB::raw('deal_log_no,user_id,user_name,agent_name,agent_cnt,bet_amount,bkge,settle_amount,fee_amount,'.$tz_sql.$zc_sql.$cb_sql.'date')])->toArray();
            } else {
                $agentInfo = DB::connection('slave')->table('unlimited_agent_bkge')->orderBy('date','desc')->whereRaw('user_name = ? and agent_name = ? and date >= ? and date < ?', [$user_name, $agent_name, $date_start, $date_end])
                    ->paginate($page_size,[DB::raw('deal_log_no,user_id,user_name,agent_name,agent_cnt,bet_amount,bkge,settle_amount,fee_amount,'.$tz_sql.$zc_sql.$cb_sql.'date')], 'page',$page)->toJson();
            }
        } elseif (!empty($agent_name)) {    //查询目标代理下所有子级代理数据
            if ($export == 1) {
                $agentInfo = DB::connection('slave')->table('unlimited_agent_bkge')->orderBy('date','desc')->whereRaw('agent_name = ? and date >= ? and date < ?', [$agent_name, $date_start, $date_end])
                    ->get([DB::raw('deal_log_no,user_id,user_name,agent_name,agent_cnt,bet_amount,bkge,settle_amount,fee_amount,'.$tz_sql.$zc_sql.$cb_sql.'date')])->toArray();
            } else {
                $agentInfo = DB::connection('slave')->table('unlimited_agent_bkge')->orderBy('date','desc')->whereRaw('agent_name = ? and date >= ? and date < ?', [$agent_name, $date_start, $date_end])
                    ->paginate($page_size,[DB::raw('deal_log_no,user_id,user_name,agent_name,agent_cnt,bet_amount,bkge,settle_amount,fee_amount,'.$tz_sql.$zc_sql.$cb_sql.'date')], 'page',$page)->toJson();
            ...
            if ($export == 1) {
                $agentInfo = DB::connection('slave')->table('unlimited_agent_bkge')->orderBy('date','desc')->whereRaw('user_name = ? and date >= ? and date < ?', [$user_name, $date_start, $date_end])
                    ->get([DB::raw('deal_log_no,user_id,user_name,agent_name,agent_cnt,bet_amount,bkge,settle_amount,fee_amount,'.$tz_sql.$zc_sql.$cb_sql.'date')])->toArray();
            } else {
                $agentInfo = DB::connection('slave')->table('unlimited_agent_bkge')->orderBy('date','desc')->whereRaw('user_name = ? and date >= ? and date < ?', [$user_name, $date_start, $date_end])
                    ->paginate($page_size,[DB::raw('deal_log_no,user_id,user_name,agent_name,agent_cnt,bet_amount,bkge,settle_amount,fee_amount,'.$tz_sql.$zc_sql.$cb_sql.'date')], 'page',$page)->toJson();
            ...
            if ($export==1) {
                $agentInfo = DB::connection('slave')->table('unlimited_agent_bkge')->orderBy('date','desc')->whereRaw('date >= :start and date < :end', ['start'=>$date_start,'end'=>$date_end])
                    ->select(DB::raw('deal_log_no,user_id,user_name,agent_name,agent_cnt,bet_amount,bkge,settle_amount,fee_amount,'.$tz_sql.$zc_sql.$cb_sql.'date'))->get()->toArray();
            } else {
                $agentInfo = DB::connection('slave')->table('unlimited_agent_bkge')->orderBy('date','desc')->whereRaw('date >= :start and date < :end', ['start'=>$date_start,'end'=>$date_end])
                    ->paginate($page_size,[DB::raw('deal_log_no,user_id,user_name,agent_name,agent_cnt,bet_amount,bkge,settle_amount,fee_amount,'.$tz_sql.$zc_sql.$cb_sql.'date')], 'page',$page)->toJson();
            }
