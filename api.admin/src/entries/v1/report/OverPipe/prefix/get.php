...
return new class() extends BaseController
{
    ...
    public function run()
    {
        ...
        return $res;
        exit;
        $games = \DB::table('game_menu')->where('switch', 'enabled')->pluck('type')->toArray();
        $res = [];
        foreach ($games as $val){
            $class = 'Logic\GameApi\Order\\'.strtoupper($val);
            if(class_exists($class) && method_exists($class, 'orderOverPipe')){
                $obj = new $class($this->ci);
                $res[] = array_merge($obj->orderOverPipe($start_date,$end_date),['user_prefix'=>$obj->getUserPrefix()]);
            }
        }
        return $res;
    }
