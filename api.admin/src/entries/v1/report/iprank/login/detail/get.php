...
return new class() extends BaseController {
    ...
    public function run() {
        ...
        $query = DB::connection('slave')->table('user as ur')
            ...
            ->where('ur.login_ip', DB::raw("INET6_ATON('".$params['ip']."')"));
        if(!empty($params['date_start'])){
            $date_start = strtotime($params['date_start']);
            $query = $query->where('ur.last_login','>=',$date_start);
        }
        if(!empty($params['date_end'])){
            $date_end = strtotime($params['date_end']) + 86399;
            $query = $query->where('ur.last_login','<=',$date_end);
        }
        $attributes['total'] = $query->count();

        $data = $query->selectRaw('ur.id as user_id,ur.name AS username,ua.uid_agent_name AS agent,le.name AS level,f.balance,ur.state')
