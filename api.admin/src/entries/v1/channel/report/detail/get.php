...
//            ->leftJoin('user_monthly_award','user_monthly_award.user_id','=','rpt_user.user_id')
        ...
        $data = $sql->groupBy(['rpt_user.count_date'])->paginate($page_size,[DB::raw( 'user.channel_id,sum(rpt_user.deposit_user_amount) as cz_amount,rpt_user.count_date,
        ...
        sum(rpt_user.turn_card_user_winnings) as zk_amount,sum(rpt_user.back_user_amount) as fyz_amount,sum(IF(rpt_user.first_deposit>0, 1, 0)) as first_recharge_user,sum(IF(rpt_user.first_deposit>0, rpt_user.deposit_user_amount, 0)) as first_recharge,sum(IF(rpt_user.first_deposit>0, rpt_user.withdrawal_user_amount, 0)) as first_withdraw,sum(IF(rpt_user.first_deposit>0, rpt_user.bet_user_amount, 0)) as first_bet,sum(IF(rpt_user.first_deposit>0, rpt_user.prize_user_amount, 0)) as first_prize')],'page',$page)->toJson();
