...
return new class extends BaseController {
    ...
    public function run($nodeId) {
        ...
        $serviceSet = (array)\DB::table('service_set')->where('node_id', $site['node_id'])->first();
        $res = $this->curlPost($serviceSet['href'],$data);
        $data = json_decode($res,true);
        if(isset($data['data']['rand'])){
            return ['url' =>$serviceSet['href2'].'?rand='.$data['data']['rand']];
        }else{
