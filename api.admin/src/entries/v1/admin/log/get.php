...
return new class() extends Action
{
    ...
    public function run()
    {
        ...
        $query = DB::connection('slave')->table('admin_logs');
        foreach ($fields as $field) {
            if (!isset($params[$field])) {
                continue;
            }
            if (isset($params['ip'])) {    //ipv6字段中有空格
                $query->whereRaw('trim(ip)=?', $params['ip']);
            } else {
