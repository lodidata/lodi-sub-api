...
return new class() extends BaseController
{
    ...
    public function run()
    {
        ...
        $flag = \DB::table('app_black_kilometre')
                ->where('channel','=',$channel)
                ->where('longitude','=',$longitude)
                ->where('latitude','=',$latitude)
                ->value('id');
        if($flag)
            return $this->lang->set(10406,['屏蔽']);
        try {
            $data = [
                'channel' => $channel,
                'longitude' => $longitude,
            ...
            $res=\DB::table('app_black_kilometre')->insert($data);
            /*============================日志操作代码================================*/
            $sta = $res !== false ? 1 : 0;
            (new Log($this->ci))->create(null , null, Log::MODULE_APP, '屏蔽设置', '屏蔽设置', "新增经纬区域", $sta, "{$channel}经纬度:{$longitude}:{$latitude},周边{$kilometre}公里");
            /*============================================================*/
