...
return new class() extends BaseController {
    ...
    const TITLE = '用户活动申请审批操作-通过';
    const QUERY = [
        'id' => 'int #申请id'
    ];

    const PARAMS = [
        'status' => 'int(required) #操作方式 0拒绝 1通过'
    ];
    const SCHEMAS = [
    ];
    ...
    public function run($id)
    {
        ...
        if (!$status) {
            ...
            //金额限制10W以内
            if (!is_numeric($params['money']) || $params['money'] >= 100000) {
                return $this->lang->set(11066);
            }
            ...
            try {
                ...
                //检查状态
                $active_inf = DB::table('active_apply')->where('id',$id)->lockForUpdate()
                    ->first();
                if($active_inf->status != 'pending' && $active_inf->status != 'undetermined'){
                    $this->db->getConnection()
                        ->rollback();
                    return $this->lang->set(10015);
                }
                ...
                $res = DB::table('active_apply')
                    ->where('id', $id)
                    ->update(['memo' => '赠送金额' . bcdiv($money,100,0), 'status' => 'pass', 'coupon_money' => $money, 'created_uid' => $admin_id, 'process_time' => date('Y-m-d H:i:s')]);
                //通过消息通知
