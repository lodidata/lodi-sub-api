...
//        $tmp = \Model\GameMenu::whereIn('id',explode(',',$data['game_ids']))->get(['id','name'])->toArray();
//        $data['game_ids'] = array_column($tmp,'name','id');
        $data['rule'] =  DB::connection('slave')->table('active_bkge_rule')->where('active_bkge_id',$id)->orderBy('min_lottery')->get([
            'rule_name',
            'min_lottery',
            'max_lottery',
            'min_deposit',
