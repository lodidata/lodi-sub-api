...
{
    const TITLE = "修改(厅主)收款银行帐号";
    const DESCRIPTION = "修改银行帐户(目前只支持状态修改)";
    ...
    const SCHEMAS = [
        [
            "id" => "int    #帐户ID",
            "status" => "enum[enabled,disabled] #状态[enabled,disabled]"
        ]
    ];
    ...
    public function run($id)
    {
        ...
        if (\Model\BankAccount::where('id', '=', $id)->update(['state' => $status])){
            /*================================日志操作代码=================================*/
            $types = [
                '1' => '银行转账',
                '2' => '支付宝',
