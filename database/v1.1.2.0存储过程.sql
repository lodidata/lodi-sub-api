CREATE DEFINER=`root`@`localhost` PROCEDURE `p_rpt_deposit_withdrawal_day`(IN `v_begin_date` varchar(15))
BEGIN
...
SET @sqlstr_05 = CONCAT(
		'UPDATE rpt_deposit_withdrawal_day rdwd
    INNER JOIN 
			(SELECT "', v_begin_date, '" count_date,ifnull(sum(is_valid_agent),0) new_valid_agent_num, ifnull(sum(new_register_deposit_num),0) agent_first_deposit_num FROM `rpt_agent`
						 WHERE count_date = "', v_begin_date, '") r 
		ON rdwd.count_date = r.count_date 
		SET rdwd.new_valid_agent_num = r.new_valid_agent_num, 
				rdwd.agent_first_deposit_num = r.agent_first_deposit_num
						 ;');
SELECT @sqlstr_05;
...
SET @sqlstr_06 = CONCAT(
		'UPDATE rpt_deposit_withdrawal_day rdwd
    INNER JOIN 
			(SELECT "', v_count_date, '" date,count( DISTINCT ( user_id ) ) AS num, IFNULL( sum( bkge ), 0 ) AS amount FROM `unlimited_agent_bkge`
						 WHERE date = "', v_count_date, '" and bkge > 0 and status =1) r 
