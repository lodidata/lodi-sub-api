   ...
   back_cnt = a.back_cnt;');
		...
		SET rdwd.shares_settle_cnt = r.num,
            ...
            SET @sqlstr_09 = CONCAT(
                    'UPDATE rpt_deposit_withdrawal_day rdwd
                INNER JOIN
                        (SELECT "', v_count_date, '" date,count( DISTINCT ( user_id ) ) AS num, IFNULL( sum( settle_amount ), 0 ) AS amount FROM `agent_loseearn_bkge`
						 WHERE count_date = "', v_count_date, '" and settle_amount > 0 and status =1) r
		ON rdwd.count_date = r.date
		SET rdwd.profit_loss_cnt = r.num,
				rdwd.profit_loss_amount = r.amount
						 ;');
            SELECT @sqlstr_09;
