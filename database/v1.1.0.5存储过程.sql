...
-- ----------------------------
-- Procedure structure for p_rpt_agent_day_bak
-- ----------------------------
DROP PROCEDURE IF EXISTS `p_rpt_agent_day_bak`;
delimiter ;;
CREATE PROCEDURE `p_rpt_agent_day_bak`(IN `v_begin_date` varchar(15))
BEGIN

	IF NOT EXISTS (SELECT 1 FROM rpt_exec_his WHERE tab_name = 'rpt_agent_day') THEN
		INSERT INTO rpt_exec_his(tab_name,exec_date) 
			SELECT 'rpt_agent_day', DATE(FROM_UNIXTIME(MIN(created), '%Y-%m-%d')) FROM user_agent;
	END IF;

	IF v_begin_date = '' THEN 
		SELECT exec_date INTO v_begin_date FROM rpt_exec_his WHERE tab_name = 'rpt_agent_day';
	END IF;

	WHILE v_begin_date <= CONVERT(CURDATE(), CHAR(12)) DO

		SET @sqlstr_01 = CONCAT(
'INSERT INTO rpt_agent_day
  (count_date,
   user_id,
   user_name,
   team_cnt,
  ...
  SELECT *
    FROM (SELECT "', v_begin_date, '" count_date,
                 ua.user_id,
                 u.`name` user_name,
                 ua.inferisors_all team_cnt,
                 (SELECT COUNT(*) FROM user_agent WHERE uid_agent = ua.user_id) child_agent_num,
                 SUM(agent_cnt_inc) agent_cnt_inc,
                 SUM(team_cnt_inc) team_cnt_inc,
                 IFNULL(SUM(first_deposit), 0) first_deposit_cnt,
                 SUM(rur.deposit_money) deposit_money,
                 SUM(rur.get_money) get_money,
                 SUM(rur.deposit_money - rur.get_money) deposit_withdraw,
                 -SUM(lottery_earnlose + live_earnlose + game_earnlose + sport_earnlose) bet_earnlose,
                 SUM(rur.coupon_money) coupon_money,
                 SUM(rur.return_money) return_money,
                 SUM(lottery_back_moeny + live_back_moeny + game_back_moeny + sport_back_moeny) total_backmoney,
                 -SUM(lottery_earnlose + live_earnlose + game_earnlose + sport_earnlose + rur.coupon_money + rur.return_money +
                      lottery_back_moeny + live_back_moeny + game_back_moeny + sport_back_moeny) total_earnlose,
           ...
           INNER JOIN `user` u
              ON ua.user_id = u.id
           INNER JOIN (SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(ua1.uid_agents, ",", rht.help_topic_id + 1), ",", -1) pid,
                             user_id cid
                        FROM user_agent ua1
                       INNER JOIN rpt_help_topic rht
                          ON rht.help_topic_id < (LENGTH(ua1.uid_agents) - LENGTH(REPLACE(ua1.uid_agents, ",", "")) + 1)
                       WHERE ua1.uid_agent <> 0
                      UNION
                      SELECT user_id pid, user_id cid FROM user_agent WHERE uid_agent = 0) t
              ON ua.user_id = t.pid
           INNER JOIN rpt_userreport rur
              ON rur.user_id = t.cid
             AND rur.count_date = "', v_begin_date, '"
            LEFT JOIN rpt_userdata rud
              ON rud.user_id = t.cid
             AND rud.count_date = "', v_begin_date, '"
           GROUP BY ua.user_id) a
      ON DUPLICATE KEY UPDATE count_date = a.count_date, user_id = a.user_id, 
   user_name = a.user_name,
   team_cnt = a.team_cnt,
   child_agent_num = a.child_agent_num,
   agent_cnt_inc = a.agent_cnt_inc,
   team_cnt_inc = a.team_cnt_inc,
   first_deposit_cnt = a.first_deposit_cnt,
   deposit_money = a.deposit_money, 
   get_money = a.get_money,
   deposit_withdraw = a.deposit_withdraw,
   bet_earnlose = a.bet_earnlose,
   coupon_money = a.coupon_money,
   return_money = a.return_money,
   total_backmoney = a.total_backmoney,
   total_earnlose = a.total_earnlose,
   lottery_valid_bet_money = a.lottery_valid_bet_money,
   lottery_win_prize = a.lottery_win_prize,
   lottery_earnlose = a.lottery_earnlose,
   lottery_back_moeny = a.lottery_back_moeny,
   live_valid_bet_money = a.live_valid_bet_money,
   live_win_prize = a.live_win_prize,
   live_earnlose = a.live_earnlose,
   live_back_moeny = a.live_back_moeny,
   game_valid_bet_money = a.game_valid_bet_money,
   game_win_prize = a.game_win_prize, 
   game_earnlose = a.game_earnlose,
   game_back_moeny = a.game_back_moeny,
   sport_valid_bet_money = a.sport_valid_bet_money,
   sport_win_prize = a.sport_win_prize,
   sport_earnlose = a.sport_earnlose,
   sport_back_moeny = a.sport_back_moeny;');
   SELECT @sqlstr_01;
	...
	UPDATE rpt_exec_his SET exec_date = DATE_ADD(v_begin_date, INTERVAL -1 DAY) WHERE tab_name = 'rpt_agent_day'; 
END
;;
delimiter ;

-- ----------------------------
-- Procedure structure for p_rpt_agent_game
-- ----------------------------
DROP PROCEDURE IF EXISTS `p_rpt_agent_game`;
delimiter ;;
CREATE PROCEDURE `p_rpt_agent_game`(IN `v_begin_date` varchar(15))
BEGIN

	DECLARE v_end_date date;
		
	IF NOT EXISTS (SELECT 1 FROM rpt_exec_his WHERE tab_name = 'rpt_agent_game') THEN
		INSERT INTO rpt_exec_his (tab_name, exec_date) 
			SELECT 'rpt_agent_game', DATE(MIN(date)) FROM orders;
	END IF;

	IF v_begin_date = '' THEN 
		SELECT exec_date INTO v_begin_date FROM rpt_exec_his WHERE tab_name = 'rpt_agent_game';
	END IF;
	...
	WHILE v_begin_date <= CONVERT(CURDATE(), CHAR(12)) DO
		...
		SET @sqlstr_01 = CONCAT(
'INSERT INTO rpt_agent_game
  (count_date,
   agent_id,
   agent_name,
   game_type,
   game_name,
   agent_game_order_cnt,
   agent_game_bet_amount,
   agent_game_prize_amount,
   agent_game_code_amount)
  SELECT *
    FROM (SELECT "', v_begin_date, '" count_date,
                 ca.pid agent_id,
                 u1.name agent_name,
                 o1.game_type game_type,
                 o1.type_name game_name,
                 COUNT(*) agent_game_order_cnt,
                 SUM(o1.bet) / 100 agent_game_bet_amount,
                 SUM(o1.send_money) / 100 agent_game_prize_amount,
                 SUM(o1.dml) / 100 agent_game_code_amount
           FROM orders o1 
          INNER JOIN user u ON u.id = o1.user_id
          INNER JOIN child_agent ca ON ca.cid = o1.user_id
           LEFT JOIN user u1 ON u1.id = ca.pid
          WHERE u.tags != 4
            AND o1.`date` = "', v_begin_date, '"
          GROUP BY ca.pid, o1.game_type) a
      ON DUPLICATE KEY UPDATE count_date = a.count_date,
   agent_id = a.agent_id, 
   agent_name = a.agent_name,
   game_type = a.game_type, 
   game_name = a.game_name,
   agent_game_order_cnt = a.agent_game_order_cnt,
   agent_game_bet_amount = a.agent_game_bet_amount,
   agent_game_prize_amount = a.agent_game_prize_amount,
   agent_game_code_amount = a.agent_game_code_amount;');
        SELECT @sqlstr_01;
		PREPARE sqlstr_01 FROM @sqlstr_01;
		EXECUTE sqlstr_01;
		DEALLOCATE PREPARE sqlstr_01;
	...
	UPDATE rpt_exec_his SET exec_date = DATE_ADD(v_begin_date, INTERVAL -2 DAY) WHERE tab_name = 'rpt_agent_game'; 
END
;;
delimiter ;
...
'INSERT INTO rpt_deposit_withdrawal_day
  ...
  SELECT *
   ...
   back_cnt = a.back_cnt;');
    ...
    INNER JOIN 
			(select  "', v_begin_date, '" count_date, count(DISTINCT user_id) deposit_user_num,sum(if(FIND_IN_SET("new",state), 1, 0)) new_deposit_user_num ,sum(if(FIND_IN_SET("offline",state), money, 0))/100  offline_amount , sum(if(FIND_IN_SET("offline",state), 1, 0)) offline_cnt,sum(if(FIND_IN_SET("online",state), money, 0))/100 online_amount,sum(if(FIND_IN_SET("online",state), 1, 0))  online_cnt, sum(if(FIND_IN_SET("tz",state), money, 0))/100 manual_deposit_amount ,sum(if(FIND_IN_SET("tz",state), 1, 0)) manual_deposit_cnt
				FROM funds_deposit 
			WHERE created >= "', v_start_time, '"
				AND created <  "', v_end_time, '" 
		...
		'UPDATE rpt_deposit_withdrawal_day rdwd
    INNER JOIN 
			(select  "', v_begin_date, '" count_date, count(DISTINCT user_id) new_register_deposit_num,sum(money) /100 new_register_deposit_amount  
				FROM funds_deposit 
			WHERE created >= "', v_start_time, '"
				AND created <  "', v_end_time, '" 
...
-- ----------------------------
-- Procedure structure for p_rpt_plat_earnlose_day
-- ----------------------------
DROP PROCEDURE IF EXISTS `p_rpt_plat_earnlose_day`;
delimiter ;;
CREATE PROCEDURE `p_rpt_plat_earnlose_day`(IN `v_begin_date` varchar(20))
BEGIN

	DECLARE v_end_date date;
  
	IF NOT EXISTS (SELECT 1 FROM rpt_exec_his WHERE tab_name = 'rpt_plat_earnlose_day') THEN
		INSERT INTO rpt_exec_his(tab_name,exec_date) 
			SELECT 'rpt_plat_earnlose_day', DATE(MIN(created)) FROM send_prize ;
	END IF;

	IF v_begin_date = '' THEN 
		SELECT exec_date INTO v_begin_date FROM rpt_exec_his WHERE tab_name = 'rpt_plat_earnlose_day';
	END IF;
  
	SET v_end_date = DATE_ADD(curdate(), INTERVAL 1 DAY);

	DELETE FROM rpt_plat_earnlose_day WHERE count_date >= v_begin_date;

	SET @sqlstr_01 = CONCAT(
'INSERT INTO rpt_plat_earnlose_day
  SELECT DATE(a.send_date),
         type_id,
         SUM(valid_money) bet_money,
         SUM(CASE WHEN prize>0 THEN prize ELSE 0 END ) win_money,
         COUNT(*) bet_num,
         COUNT(DISTINCT user_id) bet_user,
         ROUND(COUNT(*) / COUNT(DISTINCT user_id)) avg_betnum,
         SUM(valid_money) / COUNT(DISTINCT user_id) avg_betmoney,
         0, 0, 0, 0, 0, 0, NOW()
    FROM order_3th a
    JOIN `user` b
      ON a.user_id = b.id
   WHERE a.send_date >= "', v_begin_date, '"
     AND a.send_date < "', v_end_date, '"
     AND b.tags NOT IN (4, 7)
   GROUP BY DATE(a.send_date), type_id
  UNION ALL
  SELECT DATE(a.created),
         0 type_id,
         SUM(pay_money) bet_money,
         SUM(money) win_money,
         COUNT(*) bet_num,
         COUNT(DISTINCT user_id) bet_user,
         ROUND(COUNT(*) / COUNT(DISTINCT user_id)) avg_betnum,
         SUM(pay_money) / COUNT(DISTINCT user_id) avg_betmoney,
         0, 0, 0, 0, 0, 0, NOW()
    FROM send_prize a
    JOIN `user` b
      ON a.user_id = b.id
   WHERE a.created >= "', v_begin_date, '"
   ...
   GROUP BY DATE(a.created);');
	...
	SET @sqlstr_02 = CONCAT(
'INSERT INTO rpt_plat_earnlose_day
  (count_date, plat_id, in_money, in_user, avg_inmoney, create_time)
  SELECT *
    FROM (SELECT DATE(a.created) count_date,
                 IFNULL(c.id, 0) plat_id,
                 SUM(amount) in_money,
                 COUNT(DISTINCT user_id) in_user,
                 SUM(amount) / COUNT(DISTINCT user_id) avg_inmoney,
                 NOW()
            FROM funds_transfer_log a
            JOIN `user` b
              ON a.user_id = b.id
            LEFT JOIN partner c
              ON a.in_type = c.`name`
           ...
           GROUP BY DATE(a.created), IFNULL(c.id, 0)) t
      ON DUPLICATE KEY UPDATE in_money = t.in_money, in_user = t.in_user,
   avg_inmoney = t.avg_inmoney, create_time = NOW();');
	PREPARE sqlstr_02 FROM @sqlstr_02;
	EXECUTE sqlstr_02;
	DEALLOCATE PREPARE sqlstr_02;
	...
	SET @sqlstr_03 = CONCAT(
'INSERT INTO rpt_plat_earnlose_day
  (count_date, plat_id, out_money, out_user, avg_outmoney, create_time)
  SELECT *
    FROM (SELECT DATE(a.created) count_date,
                 IFNULL(c.id, 0) plat_id,
                 SUM(amount) out_money,
                 COUNT(DISTINCT user_id) out_user,
                 SUM(amount) / COUNT(DISTINCT user_id) avg_outmoney,
                 NOW()
            FROM funds_transfer_log a
            JOIN `user` b
              ON a.user_id = b.id
            LEFT JOIN partner c
              ON a.out_type = c.`name`
           WHERE a.created >= "', v_begin_date, '"
             AND a.created < "', v_end_date, '"
             AND b.tags NOT IN (4, 7)
			 AND a.status = "success"
           GROUP BY DATE(a.created), IFNULL(c.id, 0)) t
      ON DUPLICATE KEY UPDATE out_money = t.out_money,
   out_user = t.out_user, avg_outmoney = t.avg_outmoney,
   create_time = NOW();');
	PREPARE sqlstr_03 FROM @sqlstr_03;
	EXECUTE sqlstr_03;
	DEALLOCATE PREPARE sqlstr_03;

	UPDATE rpt_exec_his SET exec_date = CURDATE() where tab_name = 'rpt_plat_earnlose_day';
END
;;
delimiter ;

-- ----------------------------
-- Procedure structure for p_rpt_plat_earnlose_month
-- ----------------------------
DROP PROCEDURE IF EXISTS `p_rpt_plat_earnlose_month`;
delimiter ;;
CREATE PROCEDURE `p_rpt_plat_earnlose_month`(IN `v_begin_date` VARCHAR(15))
BEGIN

	DECLARE v_end_date DATE;
  
	IF NOT EXISTS (SELECT 1 FROM rpt_exec_his WHERE tab_name = 'rpt_plat_earnlose_month') THEN
		INSERT INTO rpt_exec_his(tab_name,exec_date) 
			SELECT 'rpt_plat_earnlose_month', DATE_ADD(DATE(MIN(created)), INTERVAL -DAY(MIN(created)) +1 DAY) FROM send_prize ;
	END IF;

	IF v_begin_date = '' THEN 
		SELECT exec_date INTO v_begin_date FROM rpt_exec_his WHERE tab_name = 'rpt_plat_earnlose_month';
	END IF;
  
	SET v_begin_date = DATE_ADD(v_begin_date, INTERVAL -DAY(v_begin_date) +1 DAY);
	
	DELETE FROM rpt_plat_earnlose_month WHERE count_date >= v_begin_date; 

	WHILE v_begin_date <= DATE_ADD(CURDATE(), INTERVAL -DAY(CURDATE())+1 DAY) DO

		SET v_end_date = DATE_ADD(v_begin_date, INTERVAL 1 MONTH);
		
		SET @sqlstr_01 = CONCAT(
'INSERT INTO rpt_plat_earnlose_month
  SELECT "', v_begin_date, '",
         type_id,
         SUM(valid_money) bet_money,
         SUM(CASE WHEN prize>0 THEN prize ELSE 0 END ) win_money,
...
'INSERT INTO rpt_plat_earnlose_month
  ...
  SELECT *
   ...
   create_time = NOW();');
		...
		SET v_begin_date=DATE_ADD(v_begin_date, INTERVAL 1 MONTH);
	END WHILE;
  
	UPDATE rpt_exec_his SET exec_date = DATE_ADD(CURDATE(), INTERVAL -DAY(CURDATE()) +1 DAY) WHERE tab_name = 'rpt_plat_earnlose_month';
END
;;
delimiter ;

-- ----------------------------
-- Procedure structure for p_rpt_plat_earnlose_week
-- ----------------------------
DROP PROCEDURE IF EXISTS `p_rpt_plat_earnlose_week`;
delimiter ;;
CREATE PROCEDURE `p_rpt_plat_earnlose_week`(IN `v_begin_date` VARCHAR(15))
BEGIN

	DECLARE v_end_date DATE;
  
	IF NOT EXISTS (SELECT 1 FROM rpt_exec_his WHERE tab_name = 'rpt_plat_earnlose_week') THEN
		INSERT INTO rpt_exec_his(tab_name,exec_date) 
			SELECT 'rpt_plat_earnlose_week', DATE(SUBDATE(MIN(created), WEEKDAY(MIN(created)))) FROM send_prize ;
	END IF;

	IF v_begin_date='' THEN 
		SELECT exec_date INTO v_begin_date FROM rpt_exec_his WHERE tab_name = 'rpt_plat_earnlose_week';
	END IF;
  
	SET v_begin_date = SUBDATE(v_begin_date, WEEKDAY(v_begin_date));

	DELETE FROM rpt_plat_earnlose_week WHERE count_date >= v_begin_date; 

	WHILE v_begin_date <= SUBDATE(CURDATE(), WEEKDAY(CURDATE())) DO

		SET v_end_date = DATE_ADD(v_begin_date, INTERVAL 7 DAY);
		
		SET @sqlstr_01 = CONCAT(	
'INSERT INTO rpt_plat_earnlose_week
  SELECT "', v_begin_date, '",
         type_id,
         SUM(valid_money) bet_money,
         SUM(CASE WHEN prize > 0 THEN prize ELSE 0 END ) win_money,
...
'INSERT INTO rpt_plat_earnlose_week
  ...
  SELECT *
   ...
   create_time = NOW();');
		...
		SET v_begin_date = DATE_ADD(v_begin_date,INTERVAL 7 DAY);
	END WHILE;

	UPDATE rpt_exec_his SET exec_date=SUBDATE(CURDATE(),WEEKDAY(CURDATE())) WHERE tab_name='rpt_plat_earnlose_week';
END
;;
delimiter ;
...
WHILE v_begin_date<=DATE_ADD(CURDATE(), INTERVAL 1 DAY)
	DO
		SET v_yesterday = DATE_ADD(v_begin_date, INTERVAL -1 DAY);
		SET v_start_time = CONCAT(v_yesterday, ' 20:00:00');
		SET v_end_time = CONCAT(v_begin_date, ' 20:00:00');
...
delimiter ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_rpt_agent`(IN `v_begin_date` varchar(15))
BEGIN
    ...
    FROM (SELECT t.*, m.balance_amount
            FROM (SELECT "', v_begin_date, '" count_date,
                         ...
                         IFNULL(SUM(IFNULL(ru.back_user_amount, 0)), 0) back_agent_amount,
												 IFNULL(SUM(IF(ru.register_time >= "', v_start_time, '" &&  ru.register_time < "', v_end_time, '",ru.deposit_user_amount,0)), 0) new_register_deposit_amount,
												 IFNULL(SUM(IF(ru.register_time >= "', v_start_time, '" &&  ru.register_time < "' , v_end_time, '" && ru.deposit_user_amount > 0,1,0)), 0) new_register_deposit_num,
