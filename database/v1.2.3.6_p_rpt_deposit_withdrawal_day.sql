...
BEGIN
    ...
    DECLARE v_count_date date;
		DECLARE v_count_start datetime;
		DECLARE v_count_end datetime;
    ...
    WHILE v_begin_date <= now_date DO
            ...
            SET v_end_time = CONCAT(v_end_date, ' 00:00:00');
						SET v_count_start = CONCAT(v_begin_date, ' 00:00:00');
            SET v_count_end = CONCAT(v_begin_date, ' 23:59:59');
            ...
            LEFT JOIN (SELECT "', v_begin_date, '" count_date,
                              ...
                              SUM(CASE WHEN fdl1.deal_type in (107, 109, 113, 701,702,703) THEN fdl1.deal_money ELSE 0 END) / 100 return_amount,
                              SUM(CASE WHEN fdl1.deal_type in (107, 109, 113, 701,702,703) THEN 1 ELSE 0 END) return_cnt,
                       ...
                       WHERE fdl1.deal_type IN (101, 102, 105, 106, 107, 109, 113, 114, 201,218, 219,204, 213, 308, 309, 501, 701,702,703)
					     AND u.tags != 4
   ...
   back_cnt = a.back_cnt;');
		...
		SET rdwd.shares_settle_cnt = r.num,
            ...
            DEALLOCATE PREPARE sqlstr_08;
						...
						SET @sqlstr_09 = CONCAT(
                    'UPDATE rpt_deposit_withdrawal_day rdwd
                INNER JOIN
(SELECT
	date,
	sum( a1.amount ) AS total_amount,
...
FROM
	agent_loseearn_bkge 
WHERE 
	 bkge_time >= "', v_count_start, '"
	and bkge_time <= "', v_count_end, '" 
	UNION
SELECT
	"', v_begin_date, '" date,
	count( DISTINCT ( user_id ) ) AS num,
...
WHERE
	...
	AND bkge_time <= "', v_count_end, '" 
		...
		ON rdwd.count_date = r.date
		SET rdwd.profit_loss_cnt = r.total_num,
				rdwd.profit_loss_amount = r.total_amount
						 ;');
            SELECT @sqlstr_09;
            PREPARE sqlstr_09 FROM @sqlstr_09;
            EXECUTE sqlstr_09;
            DEALLOCATE PREPARE sqlstr_09;
