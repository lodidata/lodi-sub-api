  ...
  SELECT *
    FROM (SELECT t.*, m.balance_amount
            FROM (SELECT "', v_begin_date, '" count_date,
                         ...
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE t1.first_recharge_status END, 0)), 0) first_deposit_cnt,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.deposit_user_amount END, 0)), 0) deposit_agent_amount,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.withdrawal_user_amount END, 0)), 0) withdrawal_agent_amount,
												 IFNULL(SUM(IFNULL(ru.manual_deduction_amount, 0)), 0) manual_deduction_amount,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.bet_user_amount END, 0)), 0) bet_agent_amount,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.prize_user_amount END, 0)), 0) prize_agent_amount,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.coupon_user_amount END, 0)), 0) coupon_agent_amount,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.return_user_amount END, 0)), 0) return_agent_amount,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.turn_card_user_winnings END, 0)), 0) turn_card_agent_winnings,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.promotion_user_winnings END, 0)), 0) promotion_agent_winnings,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE ru.back_user_amount END, 0)), 0) back_agent_amount,
                         IFNULL(SUM(IF(CASE WHEN ca.pid = ca.cid THEN 0 ELSE t1.first_recharge_status END,ru.deposit_user_amount,0)), 0) new_register_deposit_amount,
                         IFNULL(SUM(IFNULL(CASE WHEN ca.pid = ca.cid THEN null ELSE t1.first_recharge_status END, 0)), 0) new_register_deposit_num,
                         IFNULL(SUM(IF((CASE WHEN ca.pid = ca.cid THEN 0 ELSE ru.deposit_user_amount END) > 0,1,0)), 0) deposit_user_num,
          ...
          LEFT JOIN (SELECT ca.pid,
                      ...
                      INNER JOIN (SELECT pid, cid FROM child_agent UNION SELECT id pid, id cid FROM user) ca ON u.id = ca.cid
                        WHERE tags != 4
                        GROUP BY ca.pid) m
