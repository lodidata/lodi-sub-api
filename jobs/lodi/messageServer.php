...
//            }
    ...
    if ($worker->id === $proccId) {
        ...
        \Workerman\Lib\Timer::add($interval, function () {
            ...
            $minute = (int)$redis->get(\Logic\Define\CacheKey::$perfix['rebot_time_minute']);
            $week_award_day  = $redis->get(\Logic\Define\CacheKey::$perfix['week_award_day']);
            $week_award_time = $redis->get(\Logic\Define\CacheKey::$perfix['week_award_time']);

            if ($hour == date('H') && (isset($minute) && date('i') >= $minute)) {
                ...
                $app->getContainer()->logger->debug("【返水测试】");
            ...
            // 周薪
            if ($week_award_day == date('N') && date('H') == date('H', strtotime($week_award_time)) && date('i') >= date('i', strtotime($week_award_time))) {
                $app->getContainer()->logger->debug("【开始周薪】");
                //层级的月俸禄模块 @author Taylor 2018-12-28
                $award = new \Logic\Level\Award($app->getContainer());
                $award->week_award();
            }
...
//            if (1 == date('w') && date('H') == date('H', strtotime($week_issue_time))) {
            if ($week_issue_day == date('N') && date('H') == date('H', strtotime("{$week_issue_time} -2 hour"))) {
    ...
    if($worker->id === $proccId) {
        ...
        \Workerman\Lib\Timer::add($interval, function () {
            global $app;
            if (date("H") == '02') {   //凌晨2点执行
                $rptChannel = new Logic\Report\RptChannel($app->getContainer());
                $rptChannel->rptChannel();
            }
        });
    }
    ...
    if ($worker->id === $proccId) {
        ...
        $callback = function ($msg) use ($exchange, $queue, $app, $logger) {
            try {
                $logger->info("【 $exchange, $queue 】" . $msg->body);
                $params = json_decode($msg->body, true);
                $package=new \Logic\Lottery\Package($app->getContainer());
                $package->batch($params);
    ...
    if ($worker->id === $proccId) {
        ...
        $callback = function ($msg) use ($exchange, $queue, $app, $logger) {
            try {
                $logger->info("【 $exchange, $queue 】" . $msg->body);
                $logger->error("back_water: ".date('Y-m-d H:i:s'));
                $params = json_decode($msg->body, true);
                $package=new \Logic\Lottery\Backwater($app->getContainer());
                $package->sendBackWater($params);
    ...
    //   用户相关的充值、注册统计 22
    if ($worker->id === $proccId) {
        $exchange = 'user_statistics_req_dep';
        $queue = $exchange . '_1';
        ...
        $callback = function ($msg) use ($exchange, $queue, $app, $logger) {
            try {
                $logger->info("【 $exchange, $queue 】" . $msg->body);
                $params = json_decode($msg->body, true);
                $package=new \Logic\User\StatisticsRegDep($app->getContainer());
                $package->sendStatisticsRegDep($params);

            } catch (\Exception $e) {
                $logger->error('statistics error:'.$e->getMessage().' '.$msg->body);
            }
    ...
    //   用户直推月周日反水比例设置 23
    if ($worker->id === $proccId) {
        $exchange = 'user_direct_bkge_increase';
        $queue = $exchange . '_1';
        ...
        $callback = function ($msg) use ($exchange, $queue, $app, $logger) {
            try {
                ...
                $params = json_decode($msg->body, true);
                $rebet = new \Logic\Lottery\Rebet($app->getContainer());
                $rebet->updateUserDirectBkgeIncr($params);

            } catch (\Exception $e) {
                $logger->error('direct_bkge error:'.$e->getMessage().' '.$msg->body);
            }
