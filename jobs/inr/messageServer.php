...
//            if (1 == date('w') && date('H') == date('H', strtotime($week_issue_time))) {
    ...
    if ($worker->id === $proccId) {
        ...
        \Workerman\Lib\Timer::add($interval, function () {
            ...
            $config2 = \Logic\Set\SystemConfig::getModuleSystemConfig('realTimeRebate');
            //是否开启实时返佣
            if(isset($config2['switch']) && $config2['switch'] === true){
                $app->getContainer()->logger->error("【开始实时返佣】");
                $bkge = new \Logic\User\Bkge($app->getContainer());
                $bkge->realTimeRebateRunData();
            }
        });
    }
    ...
    if($worker->id === $proccId){
        ...
        \Workerman\Lib\Timer::add($interval,function (){
            if(date("H") == '00'){
                global $app;
                $app->getContainer()->logger->error("【开始成就活动统计发放】");
                $gameapi = new Logic\Lottery\Rebet($app->getContainer());
                $gameapi->sendAchievementActivity();
            }
        });
    }
    ...
    if($worker->id === $proccId) {
        ...
        \Workerman\Lib\Timer::add($interval, function () {
            if (date("H") == '00') {
                global $app;
                $app->getContainer()->logger->error("【开始今日任务自动发放】");
                $gameapi = new \Logic\GameApi\gameTask($app->getContainer());
                $gameapi->batchDrawGameTask();
            }
        });
    }
