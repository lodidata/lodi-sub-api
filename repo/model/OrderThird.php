...
class OrderThird extends \Illuminate\Database\Eloquent\Model
{
    ...
    public static function getRecords($condition, $page = 1, $pageSize = 20)
    {
        ...
        if (isset($condition['state'])) {
            switch ($condition['state']) {
                case 'winning':
                    $table = $table->whereRaw("find_in_set('winning', lottery_order.state)");
                    break;
                case 'lose':
                    $table = $table->whereRaw("find_in_set('open', lottery_order.state) AND !find_in_set('winning', lottery_order.state) ");
                    break;
                default:
                    $table = $table->whereRaw("!FIND_IN_SET('open',lottery_order.state)");
                    break;
            }
        }

        isset($condition['id']) && $table = $table->where('lottery_order.id', $condition['id']);
        isset($condition['user_id']) && $table = $table->where('lottery_order.user_id', $condition['user_id']);
        isset($condition['lottery_number']) && $table = $table->where('lottery_order.lottery_number', $condition['lottery_number']);
        isset($condition['order_number']) && $table = $table->where('lottery_order.order_number', $condition['order_number']);
        isset($condition['lottery_id']) && $table = $table->where('lottery_order.lottery_id', $condition['lottery_id']);
        isset($condition['start_time']) && $table = $table->where('lottery_order.created', '>=', $condition['start_time']);
        isset($condition['end_time']) && $table = $table->where('lottery_order.created', '<=', $condition['end_time']);
        isset($condition['room_id']) && $table = $table->where('lottery_order.room_id', $condition['room_id']);

        // 排除测试和试玩用户
        if (isset($condition['notInTags'])) {
            $notInTags = $app->getContainer()->get('settings')['websize']['notInTags'];
            $table = $table->leftjoin('user', 'user.id', '=', 'lottery_order.user_id')->whereNotIn('user.tags', $notInTags);
        }
...
//            'order_number' => 'lottery_order.order_number',
        ...
        if (isset($condition['info'])) {
            $select['period_code'] = DB::raw('(SELECT period_code FROM lottery_info WHERE lottery_number = lottery_order.lottery_number AND lottery_type = lottery_order.lottery_id) AS period_code');
        }
...
//        echo $table->toSql();exit;
    ...
    {
        $table = DB::table('lottery_order')->leftjoin('send_prize', function ($join) {
            ...
            ->selectRaw('lottery.name, count(lottery_order.id) as bet_times, sum(lottery_order.pay_money) as bet_money, sum(send_prize.pay_money) as valid_bet, sum(earn_money) as money');
