 ...
 * WEPAY代付
 */
class BIGHEADPAY extends BASES {
    private $httpCode = '';
    ...
    public function runTransfer() {
        ...
        $params = [
            ...
            "businessCode"       => $this->getBankName(),
            "accNo"        => $this->bankCard,
            "accName"       => $this->bankUserName,
            "amount"         => bcdiv($this->money, 100, 2),
            "bankCode"         => "",
            "phone" => !empty($this->mobile) ? $this->mobile : '1234567890',
            "notifyUrl"      => $this->payCallbackDomain . '/thirdAdvance/callback/bigheadpay',
            "remake"         => 'none',
        ];
        ...
        if($this->httpCode == 200) {
            if($status == 0) {
                ...
                $this->return['msg']     = 'BIGHEADPAY:' . $message ?? '代付失败';
                return;
            }
        }
    ...
    public function sign($data) {
        ...
        $originalString =  substr($originalString, 0, -1).$this->key;
    ...
    public function callbackResult($params) {
        ...
        $this->re = $this->return;
        $this->updateTransferOrder($this->money, $real_money, $this->parameter['orderNo'],//第三方转账编号
            '', $status,0,'');
    }
    ...
    private function getBankName() {
        ...
        $banks = [
            "Gcash"                                  => "100102",
            "Paymaya Philippines, Inc."              => "100065",
        ];
