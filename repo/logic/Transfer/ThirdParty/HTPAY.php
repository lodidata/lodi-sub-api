...
class HTPAY extends BASES {
    ...
    public function runTransfer() {
        ...
        $data            = [
            ...
            'code'         => $this->getBankName(),
            'name'         => $this->bankUserName,
            'account'      => $this->bankCard,
            'remark'       => 'no',
            'notify_url'   => $this->payCallbackDomain . '/thirdAdvance/callback/htpay',
        ];
        ...
        if($this->httpCode == 200) {
            if($result['status'] == '1') {
                ...
                $this->return['msg']     = 'HTPAY:' . $message ?? '代付失败';
                return;
            }
        }
    ...
    public function getTransferResult() {
        ...
        if($this->httpCode == 200) {
            ...
            } elseif($result['status'] == '3' || ($result['status'] == '0' && $message == 'Order does not exist')) {
                $status       = 'failed';
