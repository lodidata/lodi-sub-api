...
class UZPAY extends BASES
{
    ...
    public function runTransfer()
    {
        ...
        $data = [
            ...
            'back_url' => $this->payCallbackDomain . '/thirdAdvance/callback/uzpay',
        ];
        ...
        if ($this->httpCode == 200) {
            ...
            $this->return['msg'] = 'DZXUM:' . $message ?? '代付失败';
            return;
        }
    ...
    public function getTransferResult()
    {
        ...
        if ($this->httpCode == 200) {
            if ($code == 'SUCCESS') {
                ...
                if ($result['tradeResult'] == 1) {
                    $status = 'paid';
                    $this->return = ['code' => 1, 'msg' => $message, 'balance' => 0];
                } elseif ($result['tradeResult'] == 2 || $result['tradeResult'] == 3) {
                    $status = 'failed';
                    $this->return = ['code' => 0, 'msg' => $message, 'balance' => 0];
                } else {
                    $this->return = ['code' => 0, 'msg' => $message, 'balance' => 0];
                    return;
                }
        ...
        $this->return = ['code' => 886, 'transfer' => '', 'msg' => 'http_code:' . $this->httpCode . ' message:' . $message, 'balance' => 0];
    }
