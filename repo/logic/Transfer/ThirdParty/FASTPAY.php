...
class FASTPAY extends BASES {
    ...
    public function runTransfer() {
        $params          = [
            ...
            "currency"    => "PHP",            //币种
            "acc_code"    => $this->bankCode??"",   //收款银行编号
            "acc_name"    => $this->bankUserName??"",   //收款人姓名
            "acc_no"          => $this->bankCard??"",   //收款人账号
            "returnurl"       => $this->payCallbackDomain . '/thirdAdvance/callback/fastpay',   //回调地址
        ];
        ...
        if($this->httpCode == 200) {
            ...
            if($status === 'success') {
                ...
                $this->return['msg']     = 'FASTPAY:' . $message ?? '代付失败';
                return;
            }
        }
        ...
        if($this->httpCode == 200) {
            if($checkstatus === 'success') {
                ...
                if(!empty($result['currency'])) {
                    foreach($result['currency'] as $k => $v) {
