...
class BNG
{
    ...
    private function getRecord($condition)
    {
        ...
        // 遊戲類型
        isset($condition['kind_id']) && $table->where('game_id', $condition['kind_id']);
    ...
    public function records($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 子标题
            $result[$k]['sub_game_name'] =  $v['gameplat'] == 'web' ? $ci->lang->text("web") : $ci->lang->text("mobile");
            // 玩法名
            $result[$k]['play_name'] = $ci->lang->text($this->game_type);
            // 输赢
            $profit = $v['winAmount'] - $v['betAmount'];
            ...
            // 描述
            $result[$k] += $this->parseRecordsState($profit, $v['gameDate']);
        }
        return $result;
    }
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['game_name'] = $v['game_name'];
            // 桌子号 > 椅子号
            $result[$k]['TableID'] =  $v['gameplat'] == 'web' ? $ci->lang->text("web") : $ci->lang->text("mobile");
            // 总投注金额
    ...
    public function detail($condition)
    {
        ...
        // 派奖/输赢 （收入 - 支出）
        $profit = $data->winAmount - $data->betAmount;
        $result = [
            // logo
            'logo' => '',
            // 彩种名
            ...
            // 期号
            'sub_game_name' => $data->gameplat == 'web' ? $ci->lang->text("web") : $ci->lang->text("mobile"),
            // 投注金额 转为真实金额
            'pay_money' => (string)($data->betAmount),
            // 中奖金额
            'send_money' => (string)($profit < 0 ? 0 : $profit),
        ];
        $result += $this->parseRecordsState($profit, $data->gameDate);
        ...
        $detail = [
            ...
            [
                ...
                'value' => $data->gameplat == 'web' ? $ci->lang->text("web") : $ci->lang->text("mobile"),
            ],
            [
