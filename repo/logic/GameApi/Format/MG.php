...
class MG
{
    ...
    public function getRecord($condition)
    {
        ...
        if (isset($condition['order_number'])) {
            $table->where('betUID', $condition['order_number']);
        } else {
            // 订单时间过滤
            if (isset($condition['start_time']) && $condition['start_time']) {
                $table->where('createdTime', '>=', $condition['start_time']);
            } else {
                $table->where('createdTime', '>=', date('Y-m-d'));
            }

            if (isset($condition['end_time']) && $condition['end_time']) {
                $table->where('createdTime', '<=', $condition['end_time']);
            } else {
                $table->where('createdTime', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                ...
                //派彩金额
                'send_prize' => \DB::raw("sum(payoutAmount) as total_send_prize"),
                //总数
            ...
            $select = [
                ...
                'id' => 'id',
                // 游戏局号列表 基本定义为 订单号
                'order_number' => 'betUID as order_number',
                // 用户id
                'user_id' => 'user_id',
                // 中奖金额
                'income' => 'payoutAmount',
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 派奖/输赢
            $result[$k]['Profit'] = $v['payoutAmount'];
            // 抽水
            $result[$k]['Revenue'] = '-';
        }
        return $result;
    ...
    public function detail($condition)
    {
        ...
        $result = [
            ...
            // 中奖金额
            'send_money' => (string)($data->payoutAmount < 0 ? 0 : $data->payoutAmount),
        ];
        $result += self::parseRecordsState($data->payoutAmount, $data->payoutAmount);
        // 方案详情
        $detail = [
            [
            ...
            [
                ...
                'value' => bcdiv($data->payoutAmount, 100, 2)
            ],
            [
