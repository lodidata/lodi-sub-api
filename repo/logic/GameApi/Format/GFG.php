...
class GFG {
    ...
    public function getRecord($condition) {
        ...
        $table = \DB::connection('slave')->table($this->order_table)->where('game_menu_id', $this->game_id);
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                ...
                //子游戏名称
                'sub_game_name' => 'filedName as sub_game_name',
                'fee' => 'fee',
            ];
    ...
    public function records($condition, $page = 1, $pageSize = 20) {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 子游戏名称
            $result[$k]['sub_game_name'] = $v['sub_game_name'];
            // 玩法名
            $result[$k]['play_name'] = '-';
            // 输赢
            $result[$k]['profit'] = (string)($v['income']);
            // 投注金额 以分为单位
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20){
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
            $result[$k]['TableID'] =  $v['sub_game_name'];
            // 总投注金额
