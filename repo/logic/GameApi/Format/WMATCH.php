...
class WMATCH
{
    ...
    public function getRecord($condition)
    {
        ...
        $table = \DB::connection('slave')->table($this->order_table)
            ->where('gameTypeId', $this->game_id);

        // 状态
        if (isset($condition['state'])) {
            switch ($condition['state']) {
                case 'winning':
                    $table->where('totalWinAmount', '>', '0');
                    break;
                case 'lose':
                    $table->where('totalWinAmount', '<=', '0');
                    break;
        ...
        // KindID 筛选
        isset($condition['kind_id']) && $table->where('gameIdentify', $condition['kind_id']);
        ...
        if (isset($condition['order_number'])) {
            ...
            if (isset($condition['start_time']) && $condition['start_time']) {
                $table->where('roundEndTime', '>=', $condition['start_time']);
            } else {
                $table->where('roundEndTime', '>=', date('Y-m-d'));
            }

            if (isset($condition['end_time']) && $condition['end_time']) {
                $table->where('roundEndTime', '<=', $condition['end_time']);
            } else {
                $table->where('roundEndTime', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                // 总下注
                'all_bet' => \DB::raw("sum(totalBetAmount) as total_bet"),
                // 有效投注
                'cell_score' => \DB::raw("sum(totalBetAmount) as total_available_bet"),
                // 用户数量
                'user_id' => \DB::raw("count(DISTINCT user_id) as total_user_count"),
                //派彩金额
                'send_prize' => \DB::raw("sum(totalWinAmount) as total_send_prize"),
            ];
        } else {
            // 常规操作
            $select = [
                ...
                'user_id' => 'user_id',
                // 中奖金额
                'income' => 'totalWinAmount',
                // 总下注
                'AllBet' => 'totalBetAmount',
                // 游戏创建时间
                'gameDate' => 'roundStartTime',
                //游戏代码
                'gameCode' => 'gameIdentify as kind_id',
                // 游戏开始时间
                'gameStartTime' => 'roundStartTime',
                // 游戏结束时间
                'gameEndTime' => 'roundEndTime'
            ];

            //排序默认结算时间
            if (isset($condition['orderby'])) {
                $order_field = 'roundEndTime';
                switch ($condition['orderby']) {
                    case 'CellScore':
                    case 'pay_money':
                        $order_field = 'totalBetAmount';
                        break;
                    case 'Profit':
                        $order_field = 'totalWinAmount';
                        break;
                }
    ...
    public function records($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 订单号
            $result[$k]['sub_game_name'] = $this->gameType;
            // 玩法名
            $result[$k]['play_name'] = '-';
            // 输赢
            $result[$k]['profit'] = $v['totalWinAmount'];
            // 投注金额 以分为单位
            $result[$k]['pay_money'] = $v['totalBetAmount'];
            // 显示用户名
            if ($isShowUsername) {
            ...
            // 创建时间
            $result[$k]['create_time'] = $v['roundStartTime'];
            // 描述
            $result[$k] += self::parseRecordsState($v['totalWinAmount'], $v['roundEndTime']);
        }
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['user_name'] = $userLogic->getGameUserNameById($v['user_id']);
            // 投注时间
            $result[$k]['GameStartTime'] = $v['roundStartTime'];
            // 结算时间
            $result[$k]['GameEndTime'] = $v['roundEndTime'];
            // 游戏名
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
            $result[$k]['TableID'] = $this->gameType;
            // 总投注金额
            $result[$k]['pay_money'] = $v['totalBetAmount'];
            // 有效投注
            $result[$k]['CellScore'] = $v['totalBetAmount'];
            // 派奖/输赢
            $result[$k]['Profit'] = $v['totalWinAmount'];
            // 抽水
            $result[$k]['Revenue'] = '-';
    ...
    public function detail($condition)
    {
        ...
        $result = [
            ...
            'game_name' => $game_name,
            // 期号
            'sub_game_name' => $this->gameType,
            // 投注金额 转为真实金额
            'pay_money' => $data->totalBetAmount,
            // 中奖金额
            'send_money' => (string)($data->totalWinAmount < 0 ? 0 : $data->totalWinAmount),
        ];
        $result += self::parseRecordsState($data->totalWinAmount, $data->totalWinAmount);
        // 方案详情
        $detail = [
            [
            ...
            [
                ...
                'value' => bcdiv($data->totalBetAmount, 100, 2)
            ],
            [
                'key' => $ci->lang->text('Profit and loss amount') . ':',
                'value' => bcdiv($data->totalWinAmount, 100, 2)
            ],
            [
                'key' => $ci->lang->text('End time') . ':',
                'value' => $data->roundEndTime
            ],
