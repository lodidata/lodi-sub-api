...
class SGMK {
    ...
    private function getRecord($condition) {
        ...
        if(isset($condition['order_number'])){
            $table->where('ticketId', $condition['order_number']);
        }else{
            // 订单时间过滤
            if(isset($condition['start_time']) && $condition['start_time']){
                $table->where('ticketTime', '>=', $condition['start_time']);
            }else{
                $table->where('ticketTime', '>=', date('Y-m-d'));
            }

            if(isset($condition['end_time']) && $condition['end_time']){
                $table->where('ticketTime', '<=', $condition['end_time']);
            }else{
                $table->where('ticketTime', '<=', date('Y-m-d H:i:s'));
            }
        }
    ...
    public function records($condition, $page = 1, $pageSize = 20) {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['id'] = $v['id'];
            // 订单号
            $result[$k]['order_number'] = $v['ticketId'];
            // 游戏名
            ...
            $result[$k]['play_name'] = '-';
            // 输赢
            $result[$k]['profit'] = (string)($v['winLoss']);
            // 投注金额 以分为单位
            $result[$k]['pay_money'] = (string)($v['betAmount']);
            // 显示用户名
            if ($isShowUsername) {
            ...
            // 创建时间
            $result[$k]['create_time'] = $v['ticketTime'];
            // 描述
            $result[$k] += self::parseRecordsState($v['winLoss'], $v['ticketTime']);
        }
        return $result;
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20){
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['user_name'] = $userLogic->getGameUserNameById($v['user_id']);
            // 投注时间
            $result[$k]['GameStartTime'] = $v['ticketTime'];
            // 结算时间
            $result[$k]['GameEndTime'] = $v['ticketTime'];
            // 游戏名
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
    ...
    public function detail($condition) {
        ...
        $result = [
            ...
            // 彩种名
            'game_name' => $game_name,
            'sub_game_name' => $this->categoryId=='SM' ? 'slot' : ($this->categoryId=='FH' ? 'fish':($this->categoryId=='AD' ? 'ARCADE':($this->categoryId=='BC' ? 'TABLE':''))),
            // 投注金额 转为真实金额
            'pay_money' => (string)($data->betAmount),
            // 中奖金额
            'send_money' =>(string)($data->winLoss < 0 ? 0 : $data->winLoss),
        ];
        $result+= self::parseRecordsState($data->winLoss, $data->ticketTime);
        // 方案详情
        $detail = [
            [
