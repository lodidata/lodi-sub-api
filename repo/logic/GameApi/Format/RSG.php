...
class RSG
{
    ...
    public function getRecord($condition)
    {
        ...
        $table = \DB::connection('slave')->table($this->order_table)
            ->where('game_menu_id', $this->game_id);

        // 状态
        if (isset($condition['state'])) {
            switch ($condition['state']) {
                case 'winning':
                    $table->where('WinAmt', '>', '0');
                    break;
                case 'lose':
                    $table->where('WinAmt', '<=', '0');
                    break;
        ...
        // KindID 筛选
        isset($condition['kind_id']) && $table->where('GameId', $condition['kind_id']);

        // 订单筛选
        if (isset($condition['order_number'])) {
            $table->where('SequenNumber', $condition['order_number']);
        } else {
            // 订单时间过滤
            if (isset($condition['start_time']) && $condition['start_time']) {
                $table->where('PlayTime', '>=', $condition['start_time']);
            } else {
                $table->where('PlayTime', '>=', date('Y-m-d'));
            }

            if (isset($condition['end_time']) && $condition['end_time']) {
                $table->where('PlayTime', '<=', $condition['end_time']);
            } else {
                $table->where('PlayTime', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                // 总下注
                'all_bet' => \DB::raw("sum(BetAmt) as total_bet"),
                // 有效投注
                'cell_score' => \DB::raw("sum(BetAmt) as total_available_bet"),
                // 用户数量
                'user_id' => \DB::raw("count(DISTINCT user_id) as total_user_count"),
                //派彩金额
                'send_prize' => \DB::raw("sum(WinAmt) as total_send_prize"),
                //总数
            ...
            $select = [
                ...
                'id' => 'id',
                // 游戏局号列表 基本定义为 订单号
                'order_number' => 'SequenNumber as order_number',
                // 用户id
                'user_id' => 'user_id',
                // 中奖金额
                'income' => 'WinAmt',
    ...
    public function records($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 订单号
            $result[$k]['sub_game_name'] = $this->game_id == 150 ? 'slot' : 'fish';
            // 玩法名
            $result[$k]['play_name'] = '-';
            // 输赢
            $result[$k]['profit'] = $v['WinAmt'];
            // 投注金额 以分为单位
            $result[$k]['pay_money'] = $v['BetAmt'];
            // 显示用户名
            if ($isShowUsername) {
            ...
            // 创建时间
            $result[$k]['create_time'] = $v['PlayTime'];
            // 描述
            $result[$k] += self::parseRecordsState($v['WinAmt'], $v['PlayTime']);
        }
        return $result;
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['user_name'] = $userLogic->getGameUserNameById($v['user_id']);
            // 投注时间
            $result[$k]['GameStartTime'] = $v['PlayTime'];
            // 结算时间
            $result[$k]['GameEndTime'] = $v['PlayTime'];
            // 游戏名
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
            $result[$k]['TableID'] = $this->game_id == 150 ? 'slot' : 'fish';
            // 总投注金额
            $result[$k]['pay_money'] = $v['BetAmt'];
            // 有效投注
            $result[$k]['CellScore'] = $v['BetAmt'];
            // 派奖/输赢
            $result[$k]['Profit'] = $v['WinAmt']-$v['BetAmt'];
            // 抽水
            $result[$k]['Revenue'] = '-';
        }
        return $result;
    ...
    public function detail($condition)
    {
        ...
        $result = [
            ...
            'game_name' => $game_name,
            // 期号
            'sub_game_name' => $this->game_id == 150 ? 'slot' : 'fish',
            // 投注金额 转为真实金额
            'pay_money' => $data->BetAmt,
            // 中奖金额
            'send_money' => (string) ($data->WinAmt < 0 ? 0 : $data->WinAmt),
        ];
        $result += self::parseRecordsState($data->WinAmt, $data->gameDate);
        // 方案详情
        $detail = [
            [
            ...
            [
                ...
                'value' => bcdiv($data->BetAmt, 100, 2)
            ],
            [
                'key' => $ci->lang->text('Profit and loss amount') . ':',
                'value' => bcdiv($data->WinAmt, 100, 2)
            ],
            [
