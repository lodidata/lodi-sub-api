...
class JOKER {
    ...
    public function getRecord($condition) {
        //  插入查询
        $table = \DB::table($this->order_table)->where('game_id','=', $this->game_id);
    ...
    public function records($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 订单号
            $result[$k]['sub_game_name'] =  $this->game_id==59 ? 'slot' : ($this->game_id==60 ?'fish': ($this->game_id==61 ?'casino':''));
            // 玩法名
            $result[$k]['play_name'] = '-';
            // 输赢
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
            $result[$k]['TableID'] =  $this->game_id==59 ? 'slot' : ($this->game_id==60 ?'fish': ($this->game_id==61 ?'casino':''));
            // 总投注金额
            $result[$k]['pay_money'] = $v['betAmount'];
            // 有效投注
    ...
    public function detail($condition)
    {
        ...
        $result = [
            ...
            'game_name' => $game_name,
            // 期号
            'sub_game_name' => $this->game_id==59 ? 'slot' : ($this->game_id==60 ?'fish': ($this->game_id==61 ?'casino':'')),
            // 投注金额 转为真实金额
