...
class CG
{
    ...
    private function getRecord($condition)
    {
        ...
        $table = \DB::connection('slave')->table($this->order_table)
            ->where('gameCategoryType', $this->gameCategoryType);

        // 状态
        if (isset($condition['state'])) {
            switch ($condition['state']) {
                case 'winning':
                    $table->where('MoneyWin', '>', '0');
                    break;
                case 'lose':
                    $table->where('MoneyWin', '<=', '0');
                    break;
        ...
        // KindID 筛选
        isset($condition['kind_id']) && $table->where('GameType', $condition['kind_id']);

        // 订单筛选
        if (isset($condition['order_number'])) {
            $table->where('SerialNumber', $condition['order_number']);
        } else {
            // 订单时间过滤
            if (isset($condition['start_time']) && $condition['start_time']) {
                $table->where('LogTime', '>=', $condition['start_time']);
            } else {
                $table->where('LogTime', '>=', date('Y-m-d'));
            }

            if (isset($condition['end_time']) && $condition['end_time']) {
                $table->where('LogTime', '<=', $condition['end_time']);
            } else {
                $table->where('LogTime', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                // 总下注
                'all_bet' => \DB::raw("sum(BetMoney) as total_bet"),
                // 有效投注
                'cell_score' => \DB::raw("sum(ValidBet) as total_available_bet"),
                // 用户数量
                'user_id' => \DB::raw("count(DISTINCT user_id) as total_user_count"),
                //派彩金额
                'send_prize' => \DB::raw("sum(MoneyWin-BetMoney+JackpotMoney) as total_send_prize"),
                //总数
            ...
            $select = [
                ...
                'id' => 'id',
                // 游戏局号列表 基本定义为 订单号
                'order_number' => 'SerialNumber as order_number',
                // 用户id
                'user_id' => 'user_id',
                // 中奖金额
                'winLoss' => \DB::raw("(MoneyWin+JackpotMoney) as winLoss"),
                // 下注金额
                'betAmount' => 'BetMoney as betAmount',
                //有效下注金额
                'validBet' => 'ValidBet as validBet',
                // 游戏结束时间列表
                'LogTime' => 'LogTime',
                //游戏代码
                'gameCode' => 'GameType as kind_id',
            ];
    ...
    public function records($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['play_name'] = '-';
            // 输赢
            $result[$k]['profit'] = bcmul($v['winLoss']-$v['betAmount'], 100, 0);
            // 投注金额 以分为单位
            ...
            // 创建时间
            $result[$k]['create_time'] = $v['LogTime'];
            // 描述
            $result[$k] += self::parseRecordsState($result[$k]['profit'], $v['LogTime']);
        }
        return $result;
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['user_name'] = $userLogic->getGameUserNameById($v['user_id']);
            // 投注时间
            $result[$k]['GameStartTime'] = $v['LogTime'];
            // 结算时间
            $result[$k]['GameEndTime'] = $v['LogTime'];
            // 游戏名
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
            $result[$k]['TableID'] = '-';
    ...
    public function detail($condition)
    {
        ...
        $result += self::parseRecordsState($data->winLoss - $data->betAmount, $data->LogTime);
        // 方案详情
        $detail = [
            [
            ...
            [
                ...
                'value' => bcsub($data->winLoss, $data->betAmount, 2),
            ],
            [
