...
class HB
{
    ...
    public function getRecord($condition)
    {
        ...
        $table = \DB::connection('slave')->table($this->order_table)
            ->where('GameTypeId', $this->GameTypeId);

        // 状态
        if (isset($condition['state'])) {
            switch ($condition['state']) {
                case 'winning':
                    $table->where('Payout', '>', '0');
                    break;
                case 'lose':
                    $table->where('Payout', '<=', '0');
                    break;
        ...
        // KindID 筛选
        isset($condition['kind_id']) && $table->where('GameKeyName', $condition['kind_id']);

        // 订单筛选
        if (isset($condition['order_number'])) {
            $table->where('GameInstanceId', $condition['order_number']);
        } else {
            // 订单时间过滤
            if (isset($condition['start_time']) && $condition['start_time']) {
                $table->where('DtCompleted', '>=', $condition['start_time']);
            } else {
                $table->where('DtCompleted', '>=', date('Y-m-d'));
            }

            if (isset($condition['end_time']) && $condition['end_time']) {
                $table->where('DtCompleted', '<=', $condition['end_time']);
            } else {
                $table->where('DtCompleted', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                ...
                //派彩金额
                'send_prize' => \DB::raw("sum(Payout) as total_send_prize"),
                //总数
            ...
            $select = [
                ...
                'id' => 'id',
                // 游戏局号列表 基本定义为 订单号
                'order_number' => 'GameInstanceId as order_number',
                // 用户id
                'user_id' => 'user_id',
                // 中奖金额
                'income' => 'Payout',
                ...
                'gameDate' => 'DtStarted',
                //游戏代码
                'gameCode' => 'GameKeyName as kind_id',
                // 游戏开始时间
                'gameStartTime' => 'DtStarted',
                // 游戏结束时间
                'gameEndTime' => 'DtCompleted'
            ];

            //排序默认结算时间
            if (isset($condition['orderby'])) {
                $order_field = 'DtCompleted';
    ...
    public function records($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 订单号
            $result[$k]['sub_game_name'] = $this->GameTypeId == 11 ? 'slot' : ($this->GameTypeId == 8 ? 'table' : ($this->GameTypeId == 6 ? 'qp' : ''));
            // 玩法名
            $result[$k]['play_name'] = '-';
            // 输赢
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['user_name'] = $userLogic->getGameUserNameById($v['user_id']);
            // 投注时间
            $result[$k]['GameStartTime'] = $v['DtStarted'];
            // 结算时间
            $result[$k]['GameEndTime'] = $v['DtCompleted'];
            // 游戏名
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
            $result[$k]['TableID'] = $this->GameTypeId == 11 ? 'slot' : ($this->GameTypeId == 8 ? 'table' : ($this->GameTypeId == 6 ? 'qp' : ''));;
            // 总投注金额
            $result[$k]['pay_money'] = $v['Stake'];
            // 有效投注
            $result[$k]['CellScore'] = $v['Stake'];
            // 派奖/输赢
            $result[$k]['Profit'] = $v['Payout'] - $v['Stake'];
            // 抽水
            $result[$k]['Revenue'] = '-';
        }
        return $result;
    ...
    public function detail($condition)
    {
        ...
        $result = [
            ...
            'game_name' => $game_name,
            // 期号
            'sub_game_name' => $this->GameTypeId == 11 ? 'slot' : ($this->GameTypeId == 8 ? 'table' : ($this->GameTypeId == 6 ? 'qp' : '')),
            // 投注金额 转为真实金额
            'pay_money' => $data->Stake,
            // 中奖金额
            'send_money' => (string)($data->Payout < 0 ? 0 : $data->Payout),
        ];
        $result += self::parseRecordsState($data->Payout, $data->Payout);
        // 方案详情
        $detail = [
            [
            ...
            [
                ...
                'value' => bcdiv($data->Stake, 100, 2)
            ],
            [
                'key' => $ci->lang->text('Profit and loss amount') . ':',
                'value' => bcdiv($data->Payout - $data->Stake, 100, 2)
            ],
            [
                'key' => $ci->lang->text('End time') . ':',
                'value' => $data->DtCompleted
            ],
