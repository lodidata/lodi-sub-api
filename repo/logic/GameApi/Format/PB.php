...
class PB
{
    ...
    private function getRecord($condition)
    {
        ...
        if (isset($condition['order_number'])) {
            $table->where('wagerId', $condition['order_number']);
        } else {
            // 订单时间过滤
            if (isset($condition['start_time']) && $condition['start_time']) {
                $table->where('settleDateFm', '>=', $condition['start_time']);
            } else {
                $table->where('settleDateFm', '>=', date('Y-m-d'));
            }

            if (isset($condition['end_time']) && $condition['end_time']) {
                $table->where('settleDateFm', '<=', $condition['end_time']);
            } else {
                $table->where('settleDateFm', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                ...
                'id' => 'id',
                // 游戏局号列表 基本定义为 订单号
                'order_number' => 'wagerId as order_number',
                // 用户id
                'user_id' => 'user_id',
                // 中奖金额
                'winLoss' => 'winLoss as winLoss',
                // 下注金额
                'betAmount' => 'stake as betAmount',
                //有效下注金额
                'validBet' => 'turnover as validBet',
                // 游戏结束时间列表
                'gameDate' => 'settleDateFm as gameDate',
                'BetDate' => 'wagerDateFm as BetDate',
                'selection' => 'selection',
                'game_name' => 'sport as game_name',
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['Revenue'] = '-';
            // 结果
            $result[$k]['outcome'] = $v['Result']? $app->getContainer()->lang->text(self::API_RESULT_TYPE[$v['Result']]) : '';
            $result[$k]['desc'] = $v['selection'];
        }
        return $result;
    }
    ...
    public function detail($condition)
    {
        ...
        $detail = [
            [
                ...
                'value' => $ci->lang->text(self::API_ODDS_TYPE[$data->oddsFormat]),
            ],
            [
            ...
            [
                ...
                'value' => $data->Result ? $ci->lang->text(self::API_RESULT_TYPE[$data->Result]) : ''
            ],
        ];
        // 返回内容
