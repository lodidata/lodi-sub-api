...
class AT
{
    ...
    private function getRecord($condition)
    {
        ...
        // KindID 筛选
        isset($condition['kind_id']) && $table->where('productId', $condition['kind_id']);
        ...
        if (isset($condition['order_number'])) {
            ...
            if (isset($condition['start_time']) && $condition['start_time']) {
                $table->where('createdAt', '>=', $condition['start_time']);
            } else {
                $table->where('createdAt', '>=', date('Y-m-d'));
            }

            if (isset($condition['end_time']) && $condition['end_time']) {
                $table->where('createdAt', '<=', $condition['end_time']);
            } else {
                $table->where('createdAt', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                ...
                //派彩金额
                'send_prize' => \DB::raw("sum(result) as total_send_prize"),
                //总数
            ...
            $select = [
                ...
                'user_id' => 'user_id',
                // 游戏名
                'game_name' => 'game as game_name',
                // 中奖金额
                'winLoss' => 'result as winLoss',
                // 下注金额
    ...
    public function records($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 创建时间
            $result[$k]['create_time'] = $v['createdAt'];
            // 描述
            $result[$k] += self::parseRecordsState($v['winLoss'], $v['createdAt']);
        }
        return $result;
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['user_name'] = $userLogic->getGameUserNameById($v['user_id']);
            // 投注时间
            $result[$k]['GameStartTime'] = $v['createdAt'];
            // 结算时间
            $result[$k]['GameEndTime'] = $v['updatedAt'];
            // 游戏名
            $result[$k]['game_name'] = $v['game_name'];
            // 桌子号 > 椅子号
            $result[$k]['TableID'] = $v['setId'];
    ...
    public function detail($condition)
    {
        ...
        $result = [
            ...
            // 中奖金额
            'send_money' => (string)$data->winLoss,
        ];
        $result += self::parseRecordsState($data->winLoss, $data->createdAt);
        // 方案详情
        $detail = [
            [
            ...
            [
                ...
                'value' => bcdiv($data->winLoss, 100, 2)
            ],
            [
