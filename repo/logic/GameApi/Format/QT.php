...
class QT
{
    ...
    {
        //  插入查询
        $table = \DB::connection('slave')->table($this->order_table)->where('gameCategory', $this->game_type);

        // 状态
        if (isset($condition['state'])) {
            switch ($condition['state']) {
                case 'winning':
                    $table->where('totalPayout', '>', '0');
                    break;
                case 'lose':
                    $table->where('totalPayout', '<=', '0');
                    break;
        ...
        if (isset($condition['order_number'])) {
            $table->where('round_id', $condition['order_number']);
        } else {
            // 订单时间过滤
            if (isset($condition['start_time']) && $condition['start_time']) {
                $table->where('completed', '>=', $condition['start_time']);
            } else {
                $table->where('completed', '>=', date('Y-m-d'));
            }
            if (isset($condition['end_time']) && $condition['end_time']) {
                $table->where('completed', '<=', $condition['end_time']);
            } else {
                $table->where('completed', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                // 总下注
                'all_bet' => \DB::raw("sum(totalBet) as total_bet"),
                // 有效投注
                'cell_score' => \DB::raw("sum(totalBet) as total_available_bet"),
                //派彩金额
                'send_prize' => \DB::raw("sum(totalPayout-totalBet) as total_send_prize"),
                // 用户数量
            ...
            $select = [
                ...
                'id' => 'id',
                // 订单号
                'order_number' => 'round_id as order_number',
                // 用户id
                'user_id' => 'user_id',
                // 游戏平台
                'tableid' => 'tableId',
                ...
                // 中奖金额
                'win' => 'totalPayout as win',
                // 有效投注
                'valid_account' => 'totalBet as valid_account',
                // 总下注
                'AllBet' => 'totalBet as account',
                // 游戏结算
                'reckontime' => 'completed as bettime',
                //游戏代码
                'gameCode' => 'gameId as kind_id',
                //终端来源
                'gameplat' => 'gameClientType as gameplat',
            ];
            // 详情
            if (isset($condition['info'])) {
                // 訂單下注時間
                $select['billtime'] = 'initiated as bettime';
                // 局号
                $select['round'] = 'gameProviderRoundId';
            }
            ...
            if(isset($condition['orderby'])) {
                ...
                switch ($condition['orderby']) {
                    case 'pay_money':
                        $order_field = 'totalBet';
                        break;
                    case 'CellScore':
                        $order_field = 'totalBet';
                        break;
                    case 'Profit':
                        $order_field = 'totalPayout-totalBet';
                        break;
                }
    ...
    public function detail($condition)
    {
        ...
        $result = [
            ...
            'game_name' => $game_name,
            // 期号
            'sub_game_name' => $data->gameProviderRoundId,
            // 投注金额 转为真实金额
