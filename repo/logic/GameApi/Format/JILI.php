...
class JILI
{
    ...
    public function getRecord($condition)
    {
        ...
        $table = \DB::connection('slave')->table($this->order_table)
            ->where('GameCategoryId', $this->GameCategoryId);
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                ...
                'id' => 'id',
                // 游戏局号列表 基本定义为 订单号
                'order_number' => 'OCode as order_number',
                // 用户id
                'user_id' => 'user_id',
                // 中奖金额
                'income' => 'income',
    ...
    public function records($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            // 订单号
            $result[$k]['sub_game_name'] = $this->GameCategoryId==1 ? 'slot' : ($this->GameCategoryId==5 ?'fish': ($this->GameCategoryId==8 ?'casino':''));
            // 玩法名
            $result[$k]['play_name'] = '-';
            // 输赢
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
            $result[$k]['TableID'] = $this->GameCategoryId==1 ? 'slot' : ($this->GameCategoryId==5 ?'fish': ($this->GameCategoryId==8 ?'casino':''));
            // 总投注金额
            $result[$k]['pay_money'] = $v['betAmount'];
            // 有效投注
    ...
    public function detail($condition)
    {
        ...
        $result = [
            ...
            'game_name' => $game_name,
            // 期号
            'sub_game_name' => $this->GameCategoryId==1 ? 'slot' : ($this->GameCategoryId==5 ?'fish': ($this->GameCategoryId==8 ?'casino':'')),
            // 投注金额 转为真实金额
