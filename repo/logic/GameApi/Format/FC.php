...
class FC {
    ...
    private function getRecord($condition) {
        ...
        $table = \DB::connection('slave')->table($this->order_table)
            ->whereIn('gametype', $this->game_type);
        ...
        if(isset($condition['order_number'])){
            $table->where('recordID', $condition['order_number']);
        }else{
            // 订单时间过滤
            if(isset($condition['start_time']) && $condition['start_time']){
                $table->where('bdate', '>=', $condition['start_time']);
            }else{
                    $table->where('bdate', '>=', date('Y-m-d'));
            }

            if(isset($condition['end_time']) && $condition['end_time']){
                $table->where('bdate', '<=', $condition['end_time']);
            }else{
                    $table->where('bdate', '<=', date('Y-m-d H:i:s'));
            }
        }
        ...
        if (isset($condition['statistics'])) {
            ...
            $select = [
                ...
                //派彩金额
                'send_prize' => \DB::raw("sum(winlose+jppoints) as total_send_prize"),
                //总数
            ...
            $select = [
                ...
                'id' => 'id',
                // 游戏局号列表 基本定义为 订单号
                'order_num' => 'recordID as order_number',
                // 用户id
                'user_id' => 'user_id',
                // 中奖金额
                'winLoss' => 'winlose as winLoss',
                // 下注金额
                'betAmount' => 'bet as betAmount',
                //有效下注金额
                'validBet' => 'bet as validBet',
                // 游戏结束时间列表
                'bdate' => 'bdate',
                //游戏代码
    ...
    public function records($condition, $page = 1, $pageSize = 20) {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['play_name'] = '-';
            // 输赢
            $result[$k]['profit'] = bcmul($v['winLoss']+$v['jppoints'],100,0);
            // 投注金额 以分为单位
            ...
            // 创建时间
            $result[$k]['create_time'] = $v['bdate'];
            // 描述
            $result[$k] += self::parseRecordsState($result[$k]['profit'], $v['bdate']);
        }
        return $result;
    ...
    public function recordsAdmin($condition, $page = 1, $pageSize = 20)
    {
        ...
        foreach ($data ?? [] as $k => $v) {
            ...
            $result[$k]['user_name'] = $userLogic->getGameUserNameById($v['user_id']);
            // 投注时间
            $result[$k]['GameStartTime'] = $v['bdate'];
            // 结算时间
            $result[$k]['GameEndTime'] = $v['bdate'];
            // 游戏名
            $result[$k]['game_name'] = $games[$v['kind_id']] ?? '-';
            // 桌子号 > 椅子号
            $result[$k]['TableID'] = '-';
    ...
    public function detail($condition) {
        ...
        $result = [
            ...
            'game_name' => $game_name,
            // 期号
            'sub_game_name' => $this->game_id == 93 ? 'slot' : 'fish',
            // 投注金额 转为真实金额
            'pay_money' => bcmul($data->betAmount,100,0),
            // 中奖金额
            'send_money' =>bcmul($data->winLoss + $data->jppoints,100,0),
        ];
        $result+= self::parseRecordsState($result['send_money'], $data->bdate);
        // 方案详情
        $detail = [
            [
            ...
            [
                'key' => $ci->lang->text('jackpot points').':',
                'value' => $data->jppoints
            ],
            [
