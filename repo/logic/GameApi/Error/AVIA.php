<?php
namespace Logic\GameApi\Error;

use Logic\GameApi\GameApi;
use Logic\Logic;

/**
 * game_order_error表操作
 * Class AVIA
 * @package Logic\GameApi\Error
 */
class AVIA extends Logic
{
    public function addOrder()
    {
        $error_list = \DB::table('game_order_error')
            ->where('game_type', 'AVIA')
            ->limit(1000)
            ->get(['id','order_number', 'json'])->toArray();
        foreach ($error_list as $value) {
            $value = (array)$value;
            $val = json_decode($value['json'], true);
            try {
                $is_insert = \DB::table('game_order_avia')->insert($val);
                if ($is_insert) {
                    //拉取订单其它后续 逻辑 处理通知
                    \Utils\MQServer::send(GameApi::$synchronousOrderCallback, [
                        'user_id' => $val['user_id'],
                        'game' => 'ESPORTS',
                        'order_number' => $val['OrderID'],
                        'game_type' => 'AVIA',
                        'type_name' => 'AVIA',
                        'game_id' => 96,
                        'server_id' => 0,
                        'account' => $val['UserName'],
                        'bet' => bcmul($val['BetAmount'], 100, 0),
                        'profit' => bcmul($val['Money'], 100, 0),
                        'date' => $val['RewardAt'],
                    ]);
                    \DB::table('game_order_error')
                        ->where('id', $value['id'])->delete();
                }
            } catch (\Exception $e) {
                if(\DB::table('game_order_avia')->where('OrderID', $value['order_number'])->count()){
                    \DB::table('game_order_error')
                        ->where('id', $value['id'])->delete();
                }
            }
        }
    }
}