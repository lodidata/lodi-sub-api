...
class JILI extends \Logic\GameApi\Api
{
    protected $url;
    ...
    public function childCreateAccount(string $account, string $password)
    {
        ...
        $res = $this->requestParam('CreateMember', $data);
        //0成功 101账户已经存在
    ...
    public function getJumpUrl(array $params = [])
    {
        ...
        try {
            $data = [
                ...
                'Lang' => $this->langs[LANG] ?? $this->langs['en-us'],//'th-TH',zh-CN,en-US
            ];
            $options = [
                'HomeUrl' => $back_url,
                'platform' => $is_app ? 'app' : 'web',
            ];
            //进入游戏
            //$res = $this->requestParam('Login', $data, false);
            //返回游戏位置
            $res = $this->requestParam('LoginWithoutRedirect', $data, false, $options);
            if ($res['ErrorCode'] == 0) {
                //余额转入第三方
    ...
    public function getThirdWalletBalance()
    {
        ...
        $res = $this->requestParam('GetMemberInfo', $data);
        if (isset($res['ErrorCode']) && $res['ErrorCode'] == 0) {
            if (isset($res['Data'][0]['Balance'])) {
                return [bcmul($res['Data'][0]['Balance'], 100, 0), bcmul($res['Data'][0]['Balance'], 100, 0)];
            }
        }
    ...
    public function synchronousChildData()
    {
        ...
        foreach ($data as $key => $val) {
            ...
            //不存在分类默认为电子
            if (!isset($platformTypes[$val['GameCategoryId']])) {
                $val['GameCategoryId'] = 1;
            }
            $batchData[] = $val;
    ...
    public function quitChildGame()
    {
        ...
        $result = $this->requestParam('KickMember', $data);
    ...
    public function transfer($balance, $tradeNo, $type = 'IN')
    {
        ...
        $data = [
            'Account' => $account['account'],
            'TransactionId' => $this->config['cagent'] . $tradeNo,
            'Amount' => $balance,
            'TransferType' => $type == 'OUT' ? 3 : 2,
            //'Time'          => date('Y-m-d H:i:s')
        ];
        $res = $this->requestParam('ExchangeTransferByAgentId', $data);
        ...
        if ($res['ErrorCode'] == 20) {
            $this->quitChildGame();
            return false;
        } elseif ($res['ErrorCode'] == 0 && $res['Data']['Status'] == 1) {
            return true;
        } else {
            return false;
        }
     ...
     * @return array|string
     */
    public function requestParam(string $action, array $param, bool $is_post = true, $param2 = null)
    {
        ...
        $keyG = md5(date('ymj') . $this->config['cagent'] . $this->config['key']);
        date_default_timezone_set($default_timezone);
        $querystring = '';
        if ($param) {
            $querystring .= urldecode(http_build_query($param, '', '&')) . '&';
        }
        $querystring .= 'AgentId=' . $this->config['cagent'];
        $md5string = md5($querystring . $keyG);
        $key = str_random(6) . $md5string . str_random(6);

        if ($param2) {
            $querystring .= '&' . urldecode(http_build_query($param2, '', '&'));
            $param = array_merge($param, $param2);
        }
        $url = $this->config['apiUrl'] . $action . '?' . $querystring . '&Key=' . $key;
        if ($is_post) {
            $re = Curl::commonPost($url, null, null, null, true);
        } else {
        ...
        GameApi::addRequestLog($url, 'JILI', $param, json_encode($re, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
        $ret = [];
        if ($re['status'] == 200) {
            $ret = $re['content'];
            ...
            $ret['responseStatus'] = false;
            $ret['Message'] = $re['content'] ?? 'api error';
        }
        return $ret;
    }
