...
class HOPEPAY extends BASES {
    ...
    public function initParam() {
        ...
        $data = array(
            ...
            'callbackUrl' => $this->payCallbackDomain . '/pay/callback/hopepay',
        );

        $data['sign']    = $this->sign($data);
        $this->parameter = json_encode($data);
        $this->payUrl   .= '/ext_api/v1/payment/add';
    }
...
//        echo '<pre>';print_r($this->parameter);exit;
    ...
    {
        $config    = Recharge::getThirdConfig('hopepay');
        $this->key = $config['key'];

        if(!isset($param['orderStatus']) || $param['orderStatus'] != 1){
        ...
        $res = [
            'status'        => 0,
            'order_number'  => $params['merTradeNo'],
            'third_order'   => $params['orderNo'],
            'third_money'   => bcmul($params['actualAmount'], 100),
            'third_fee'     => 0,
            'error'         => '',
        ];
    ...
    public function sign($param) {
        ...
        if (!empty($newParam)) {
            ...
            $originalString = implode('&', $sortParam) . '&signKey='.$this->key;
        } else {
    ...
    {
        $config     = Recharge::getThirdConfig('powerpay');
        $this->key  = $config['key'];
