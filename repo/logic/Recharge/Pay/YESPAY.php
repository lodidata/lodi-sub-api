...
class YESPAY extends BASES {
    ...
    public function initParam() {
        ...
        $data           = [
            ...
            'product'      => $this->rechargeType,  //ThaiP2P,ThaiQR
        ];
    ...
    public function returnVerify($param = []) {
        $config       = Recharge::getThirdConfig('yespay');
        $this->key    = $config['key'];
    ...
    public function register($userInfo)
    {
        ...
        $this->formPost();
        $result = json_decode($this->re, true);
        Recharge::addLogByTxt(['third' => 'yespay register', 'date' => date('Y-m-d H:i:s'), 'json' => json_encode($data), 'response' => $this->re], '');
