...
class FASTPAY extends BASES {
    ...
    public function initParam() {
        ...
        $data['returnurl'] = $this->payCallbackDomain . '/pay/callback/fastpay';
    ...
    {
        $config    = Recharge::getThirdConfig('fastpay');
        $this->pubKey = $config['pub_key'];

        $params = [
            'mer_no' => $param['mer_no'],
            'order_no' => $param['order_no'],
            'paytypecode' => $param['paytypecode'],
            'order_amount' => $param['order_amount'],
            'order_realityamount' => $param['order_realityamount'],
            'status' => $param['status'],
            'sign' => $param['sign'],
        ];
    ...
    public function supplyOrder($order_number, $payNo = '')
    {
        ...
        //请求参数 Request parameter
        $data = array(
            'mer_no' => $config['partner_id'],//    是   string  商户号 business number
        ...
        if ($this->http_code == 200) {
            if($checkstatus == 'success'){
                ...
                if($result['resultstatus'] != 'success'){
                    throw new \Exception($result['resultstatus']);
                }
                $res = [
                    'status'       => $result['resultstatus'],
                    'order_number' => $result['order_no'],
                    'third_order'  => '',
                    'third_money'  => $result['order_realityamount'] * 100,
                ];
                return $res;
            }
        }

        throw new \Exception('http_code:'.$this->http_code.' code:'.$checkstatus.' message:'.$message);
    }
