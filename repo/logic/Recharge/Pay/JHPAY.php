...
//        }
        $data = array(
            ...
            'pay_bankcode'    => 933,
            'pay_notifyurl'   => $this->payCallbackDomain . '/pay/callback/jhpay',
            'pay_callbackurl' => '',
            'pay_amount'      => $this->money / 100,
        );

        $data['pay_md5sign']    = $this->sign($data);
        $data['pay_productname'] = 'product';
        $data['type'] = 'json';
        $this->parameter = $data;
...
//        echo '<pre>';print_r($this->parameter);exit;
    ...
    {
        $config    = Recharge::getThirdConfig('jhpay');
        $this->key = $config['key'];
        $params = $param;
        ...
        $res = [
            ...
            'third_money'   => $params['true_amount'] * 100,
            'third_fee'     => 0,
            'error'         => '',
        ];
