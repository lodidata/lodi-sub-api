...
class WJHTPAY extends BASES {
    ...
    public function initParam() {
        ...
        $data = array(
            ...
            'callback_url' => $this->payCallbackDomain . '/pay/callback/wjhtpay',
...
//        echo '<pre>';print_r($this->parameter);exit;
    ...
    {
        $config    = Recharge::getThirdConfig('wjhtpay');
        $this->key = $config['key'];
    ...
    public function supplyOrder($order_number, $payNo = '')
    {
        ...
        $result  = json_decode($this->re, true);
        $code    = isset($result['status']) ? $result['status'] : 0;//交易狀態：0-错误 1-等待中，2,6-进行中，3-失败，5-成功。
        $message = isset($result['message']) ? $result['message'] : 'errorMsg:'.(string)$this->re;

        if ($this->http_code == 200) {
            //未支付
            if($code != 5){
                throw new \Exception($result['paymentStatus']);
            }
            if($code == 5){
