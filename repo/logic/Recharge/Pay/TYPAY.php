...
class TYPAY extends BASES {
    ...
    public function initParam() {
        ...
        $data = array(
            ...
            'notifyUrl'  => $this->payCallbackDomain . '/pay/callback/typay',
            'channel'    => $this->rechargeType,
            'number'     => '',
        );
        $data['sign']    = $this->sign($data);
        $this->parameter = $data;
        $this->payUrl   .= '/export/tiga/api/order/create';
    }
    ...
    {
        $config    = Recharge::getThirdConfig('typay');
        $this->key = $config['key'];
        //支付状态,0-订单生成,1-支付成功,2-支付失败
        if(!isset($param['payStatus']) || $param['payStatus'] == 'N'){
        ...
        $res = [
            'status'        => 0,
            'order_number'  => $params['mchOrderId'],
            'third_order'   => $params['platformOrderId'],
    ...
    public function sign($param) {
        ...
        return md5(md5(md5($originalString)));
    }
