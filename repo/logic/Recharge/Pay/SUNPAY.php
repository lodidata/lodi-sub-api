...
class SUNPAY extends BASES
{
    ...
    public function initParam()
    {
        ...
        $data = array(
            ...
            'notify_url' => $this->payCallbackDomain . '/pay/callback/sunpay',
...
//        var_dump($data_string);
    ...
    public function parseRE()
    {
        ...
        if ($this->http_code == 200 && $code == 'SUCCESS') {
            $pay_no = '';
            //下单成功，跳转支付链接 The order is successfully placed, jump to the payment link
            if ($result['tradeResult'] == '1') {
                $code = 0;
                $targetUrl = $result['payInfo'];
                $pay_no = $result['orderNo'];
            } else {
                $code = 886;
                $targetUrl = '';
            }
    ...
    {
        $config = Recharge::getThirdConfig('sunpay');
        $this->key = $config['key'];
