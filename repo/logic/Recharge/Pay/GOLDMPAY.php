...
class GOLDMPAY extends BASES {
    ...
    public function initParam() {
        ...
        if(empty($this->rechargeType)){
            $this->rechargeType = '6001';
        }
        $data = [
            'sid' => $this->partnerID,
            'outTradeNo' => $this->orderID,
            'amount' => bcdiv($this->money, 100, 2),
            'notifyUrl' => $this->payCallbackDomain . '/pay/callback/goldmpay',
            'payType' => $this->rechargeType,
        ];
        $data['sign'] = $this->sign($data);
        $this->parameter = $data;
        $this->payUrl .= '/v1/project-api/order/create';
    }
    ...
    public function returnVerify($param = []) {
        $config       = Recharge::getThirdConfig('goldmpay');
        $this->key    = $config['key'];
        $this->pubKey = $config['pub_key'];

        $res = [
            'status'       => 0,
            'order_number' => $param['userOrderNo'],
            'third_money'  => $param['payAmount'] * 100,
