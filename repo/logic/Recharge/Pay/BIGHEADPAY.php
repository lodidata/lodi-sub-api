...
class BIGHEADPAY extends BASES {
    ...
    public function initParam() {
        ...
        $data = array(
            ...
            'notifyUrl'    => $this->payCallbackDomain . '/pay/callback/bigheadpay',
            'pageUrl'      => $this->payCallbackDomain,
            'subject'      => 'none',
        );
    ...
    public function parseRE() {
        ...
        if ($this->http_code  == 200) {
            ...
            if($status == 0){
                $code = 0;
                $targetUrl = $result['data']['orderData'];
            }else{
                $code = 1;
                $targetUrl = '';
            }
    ...
    {
        $config    = Recharge::getThirdConfig('bigheadpay');
        $this->key = $config['key'];
        //支付状态,0-订单生成,1-支付成功,2-支付失败
        if(!isset($param['status']) || $param['status'] == 'fail'){
        ...
        $res = [
            'status'        => 0,
            'order_number'  => $params['merOrderNo'],
