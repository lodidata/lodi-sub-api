...
class KKPAY extends BASES
{
    ...
    public function initParam()
    {
        ...
        $data = array(
            ...
            'pay_notifyurl'   => $this->payCallbackDomain . '/pay/callback/kkpay',
            'pay_callbackurl' => $this->returnUrl ?: 'noreturn',
            'pay_amount'      => bcdiv($this->money, 100, 2),
            'pay_productname' => 'product'
        );
...
//        echo '<pre>';print_r($this->parameter);exit;
    ...
    public function parseRE()
    {
        ...
        if ($this->http_code == 200) {
            ...
            if ($code == 'success') {
                $code = 0;
                $targetUrl = $result['H5_url'];
                $this->return['pay_no'] = $result['out_trade_id'];
            } else {
                $code = 886;
                $targetUrl = '';
            }
    ...
    {
        $config = Recharge::getThirdConfig('kkpay');
        $this->key = $config['key'];
        $params = $param;
    ...
    {
        unset($param['sign'], $param['pay_productname']);
