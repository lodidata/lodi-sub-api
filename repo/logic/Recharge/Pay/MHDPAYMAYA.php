 ...
 * mhdpaymaya
 */
class MHDPAYMAYA extends BASES
{
    public $http_code;

    static function instantiation()
    {
        return new MHDPAYMAYA();
    }
    ...
    public function initParam()
    {
        ...
        $config = json_decode($this->data['channel_params'], true);
        $paymentType = $config['payment_type'] ?? 3;
        ...
        $data = array(
            ...
            'callback_url' => $this->payCallbackDomain . '/pay/callback/mhdpaymaya',
...
//        $message    = isset($result['message']) ? $result['message'] : 'errorMsg:'.(string)$this->re;
    ...
    {
        $config = Recharge::getThirdConfig('mhdpaymaya');
        $this->key = $config['key'];
