...
class METAPAY extends BASES {
    ...
    public function initParam() {
        ...
        $data = array(
            ...
            'VERSION'                => 1,
            'NOTIFY_URL'             => $this->payCallbackDomain . '/pay/callback/metapay',
//            'NOTIFY_URL'             => 'https://lodi-api-www.caacaya.com/pay/callback/metapay',
        );

        $data['SIGNED_MSG']    = $this->sign($data);
        $this->parameter = $data;
        $this->payUrl   .= '/gateway/api-center/pay-apply';
    }
...
//        echo '<pre>';print_r($this->parameter);exit;
    ...
    {
        $config    = Recharge::getThirdConfig('metapay');
        $this->key = $config['key'];
        ...
        $res = [
            'status'        => 0,
            'order_number'  => $params['TRAN_CODE'],
            'third_order'   => $params['SYS_CODE'],
            'third_money'   => $params['ORIG_AMT'] * 100,
            'third_fee'     => 0,
            'error'         => '',
        ];

        //检验状态
        if ($param['SIGNED_MSG'] == $this->sign($params)) {
            $res['status'] = 1;
        } else {
