...
class SULPAY extends BASES
{
    ...
    public function initParam()
    {
        if(empty($this->rechargeType)){
            $this->rechargeType = '40000603';
        }
        $data = array(
            'payKey'           => $this->partnerID,
            'productType'      => $this->rechargeType,
            'orderPrice'       => bcdiv($this->money, 100, 2),
            'outTradeNo'       => $this->orderID,
            'payBankAccountNo' => '1234567890',
            'productName'      => 'recharge',
            'orderIp'          => \Utils\Client::getIp(),
            'orderTime'        => date('YmdHis',time()),
            'notifyUrl'        => $this->payCallbackDomain . '/pay/callback/sulpay',
            'returnUrl'        => 'noreturn',
        );

        $data['sign'] = $this->sign($data);
        $this->parameter = $data;
        $this->payUrl .= '/quickGateWayPay/initPay';
    }
...
//        var_dump($data_string);
    ...
    public function parseRE()
    {
        ...
        if ($this->http_code == 200) {
            //下单成功，跳转支付链接
            $preg_str = '/window\.location\.href="(.*?)"/';
            preg_match($preg_str, $this->re, $url_arr);
            if(!empty($url_arr) && isset($url_arr[1])){
                $code = 0;
                $targetUrl = $url_arr[1];
                $pay_no = '';
            } else {
    ...
    {
        $config = Recharge::getThirdConfig('sulpay');
        $this->key = $config['key'];

        $params = $param;

        if (!isset($params['tradeStatus']) || ($params['tradeStatus'] != "SUCCESS" && $params['tradeStatus'] != "FINISH")) {
        ...
        $res = [
            ...
            'third_order' => $params['trxNo'],
            'third_money' => bcmul($params['orderPrice'], 100),
            'third_fee' => 0,
            'error' => '',
        ];
