...
class YOUPAY extends BASES {
    ...
    public function initParam() {
        ...
        if(is_numeric($this->rechargeType)){
            ...
            if(!empty($config_params) && isset($config_params['channelNo'])){
                $channelNo = $config_params['channelNo'];
            }
        }

        //请求参数 Request parameter
        $time = time();
        $true_name = BankUser::where('user_id',$this->userId)->where('state','enabled')->value('name');
        $data = array(
            ...
            'discount'          => null,
            'payeeName'         => $true_name ? $true_name : 'aabbcc',
            'bankName'          => null,
            'extra'             => null,
            'datetime'          => date('Y-m-d H:i:s', $time),
            'notifyUrl'         => $this->payCallbackDomain .'/pay/callback/youpay',
            'time'              => $time,
    ...
    public function returnVerify($param = []) { 
        ...
        $config    = Recharge::getThirdConfig('youpay');
        $this->key = $config['key'];
