 ...
 * @author
 */
class MASAYAPAY extends BASES
{
    public $http_code;

    static function instantiation()
    {
        return new MASAYAPAY();
    }
    ...
    public function initParam()
    {
        ...
        $data = array(
            ...
            'notifyUrl' => $this->payCallbackDomain . '/pay/callback/masayapay',
            'type' => 'gcash',
            'extend' => '',
        );
    ...
    {
        $config = Recharge::getThirdConfig('masayapay');
        $this->key = $config['key'];
    ...
    public function supplyOrder($order_number, $payNo = '')
    {
        ...
        $data['sign'] = $this->sign($data);
        $this->parameter = $data;
        $this->payUrl = $config['payurl'] . '/payin/query';
        ...
        if ($this->http_code == 200) {
            if ($code === 0) {
                ...
                $res = [
                    ...
                    'order_number' => $result['data']['outTradeNo'],
                    'third_order' => $result['data']['sn'],
