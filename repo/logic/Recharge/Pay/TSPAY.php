...
class TSPAY extends BASES {
    ...
    public function initParam() {
        ...
        $data = array(
            ...
            'type'         => 7,
            'amount'       => $this->money,
            'callback_url' => $this->returnUrl??'no return_url',
            'notify_url'   => $this->payCallbackDomain . '/pay/callback/tspay',
            'version'      => 'v1.0',
            'currency'     => '',
        );
        ...
        if(!empty($config_params) && isset($config_params['currencyCode'])){
            $data['currency'] = $config_params['currencyCode'];
        }

        $data['sign']    = $this->sign($data);
        $this->parameter = $data;
        $this->payUrl   .= '/appclient/getUrl.do';
    }
...
//        echo '<pre>';print_r($this->parameter);exit;
    ...
    public function parseRE() {
        ...
        if ($this->http_code  == 200) {
            ...
            $this->return['pay_no'] =!empty($result['data']['order_no']) ? $result['data']['order_no'] : '';
    ...
    {
        $config    = Recharge::getThirdConfig('tspay');
        $this->key = $config['key'];

        if(!isset($param['data']) || $param['code'] != "0"){
            throw new \Exception('unpaid');
        }
        $params = $param['data'];

        //兼容订单金额变化
        if(strpos($params['out_trade_no'],'new') !== false){
            $params['out_trade_no'] = explode('_',$params['out_trade_no'])[0];
            //修改订单金额
            $this->updateOrderMoney($params['out_trade_no'], $params['amount']);
        }
