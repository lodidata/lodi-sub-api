 ...
 * 验证码模块
 */
class Test extends \Logic\Logic {
...
//        }
        ...
        try {
            ...
            echo "last_fund:" .$last_fund;
            echo "<pre/>";
            $ttl = $this->ci->redis->ttl('sendMsg_'.$mobile);

            echo 'ttl : '. $ttl;echo "<pre/>";
//            if($last_fund && $this->ci->redis->ttl('sendMsg_'.$mobile) < ($loopTime * ($count-1) + $interval * ($count-1))){
            ...
            echo "当前 ： " .$fund;
            echo "<pre/>";
            echo "expire_time : ".($loopTime+$interval) * $count;
            $this->ci->redis->setex('sendMsg_'.$mobile,$expireTime,$fund);

            $fund = 'sendMsgBy'.$fund;
            $res = $this->sendMsgByShouYi($mobile, $content);
...
//                return $this->lang->set(102);
     ...
     */
    protected function sendMsgByShouYi($moblie,$content){
