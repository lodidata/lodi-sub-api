...
class GeoIP {
    ...
    public function geoip_open($filename = false, $flags) {
        if ($filename == false) {
            $filename = realpath(__DIR__ . '/../lib/service/GeoIP.dat');
        }
    ...
    public function _geoip_seek_country($gi, $ipnum) {
        ...
        for ($depth = 31; $depth >= 0; --$depth) {
            ...
            } else if ($gi->flags & GEOIP_SHARED_MEMORY) {
    ...
    public function _get_region($gi, $ipnum) {
        if ($gi->databaseType == GEOIP_REGION_EDITION_REV0) {
            $seek_region = $this->_geoip_seek_country($gi, $ipnum) - GEOIP_STATE_BEGIN_REV0;
            if ($seek_region >= 1000) {
                $country_code = "US";
            ...
            $seek_region = $this->_geoip_seek_country($gi, $ipnum) - GEOIP_STATE_BEGIN_REV1;
            //print $seek_region;
