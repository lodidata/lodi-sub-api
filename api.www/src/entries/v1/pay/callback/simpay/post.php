...
return new class extends Action
{
    ...
    public function run() {
        ...
        $log    = $params;
        \Logic\Recharge\Recharge::addLogByTxt(['third' => $pay_type, 'date' => date('Y-m-d H:i:s'), 'json' => $params, 'response' => ''], 'pay_callback');
        $data = $params ?? [];
        if ($data) {
            $desc = '';
            $flag = 0;
            ...
            if (!$pay->existThirdClass($pay_type)) {
                ...
                try{
                    $re   = $pay->returnVerify($pay_type, $data);
                    $flag = 1;
                }catch (\Throwable $e){
                    $desc = $e->getMessage();
                }
