...
return new class extends Action {
    const TOKEN = true;
    const TITLE = "GET 获取线下支付通道";
    const DESCRIPTION = "金额限制都按分来比较";
    const TAGS = "充值提现";
    const QUERY = [
       "type" => "enum[wx,unionpay,alipay,qq,jd](required,wx) #支付方式1网银支付:unionpay,2支付宝:alipay,3微信:wx,4QQ:qq,5京东:jd",
       "money" => "int(required) #支付金额 单位（分）",
   ];
    const SCHEMAS = [
           [
    ...
    public function run()
    {
        ...
        if (empty($type) || !isset($payList[$type])){
            return $this->lang->set(13);
        }
        //是否开启小数点
        $deposit_float_point = \Logic\Set\SystemConfig::getModuleSystemConfig('recharge')['deposit_float_point'] ?? true;
        ...
        $offline = $pay->offline($type,$userLevel) ?? [];
        $result = array();
            ...
            if($money >= $val['limit_once_min']  && $money <= $val['limit_once_max']
                ...
                if($deposit['day_money']+$money <= $val['limit_day_max'] || $val['limit_day_max'] == 0
                    ...
                    if($deposit['sum_money']+$money <= $val['limit_max'] || $val['limit_max'] == 0){
                        $result[$j]['pay_name'] = $this->lang->text("channel").$names[$j];
                        $result[$j]['id'] = $val['id'];
                        $result[$j]['bank_id'] = $val['bank_id'];
                        $result[$j]['bank_name'] = $this->lang->text($val['code']);
                        $result[$j]['bank_img'] = showImageUrl($val['bank_img']);
                        $result[$j]['card'] = \Utils\Utils::RSADecrypt($val['card']);
                        $result[$j]['name'] = $val['name'];
                        $result[$j]['code'] = $val['code'];
                        $result[$j]['qrcode'] = showImageUrl($val['qrcode']);
